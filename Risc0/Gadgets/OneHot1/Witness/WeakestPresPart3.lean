...
def part3_state (st: State) : State :=
   (withEqZero (Option.get! st.felts[{ name := "%7" }]!) st) 
...
lemma part3_wp {st : State} {y0 y1 y2 y3 y4 y5 y6 y7 y8 y9 y10 y11 y12 y13 y14 y15 y16 y17 y18 y19 : Option Felt} :
  Code.getReturn (MLIR.runProgram (Code.part3;dropfelt ⟨"%7"⟩) st) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] ↔
  ...
  Code.getReturn
        (part3_state_update
          ({
              ...
              constraints := [x0 = 0 → (1 - if x0 = 0 then 1 else 0) = 0, x0 = 0], cycle := 0, felts := Map.empty,
              isFailed := false, props := Map.empty,
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%7" }] ←
            (if x0 = 0 then 1 else 0) - 1)) =
...
lemma closed_form {x0: Felt} :
  Code.run (start_state [x0]) = [y0,y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12,y13,y14,y15,y16,y17,y18,y19] ↔
   Code.getReturn (Γ (part2_drops (part2_state st)) ⟦Code.part3; dropfelt { name := "%7" }⟧) =
      [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19]  := by
