...
def part0_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%2" }] ← True)[felts][{ name := "%0" }] ← 0)["%3"] ←ₛ
            getImpl st { name := "code" } 0 0)[felts][{ name := "%4" }] ←
          -Option.get!
              (State.felts
                (((st[props][{ name := "%2" }] ← True)[felts][{ name := "%0" }] ← 0)["%3"] ←ₛ
                  getImpl st { name := "code" } 0 0)
                { name := "%3" })) 
...
def part0_state_update (st: State): State :=
  Γ (part0_drops (part0_state st)) ⟦Code.part1;dropfelt ⟨"%4"⟩;Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩⟧

-- Prove that substituting part0_state for Code.part0 produces the same result
lemma part0_wp {st : State} :
  Code.getReturn (Γ st ⟦Code.part0;dropfelt ⟨"%0"⟩;dropfelt ⟨"%3"⟩;Code.part1;dropfelt ⟨"%4"⟩;Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩⟧)↔
  Code.getReturn (part0_state_update st) := by
  generalize eq : (dropfelt ⟨"%0"⟩;dropfelt ⟨"%3"⟩;Code.part1;dropfelt ⟨"%4"⟩;Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩) = prog
  unfold Code.part0
  MLIR
  rewrite [←eq]
