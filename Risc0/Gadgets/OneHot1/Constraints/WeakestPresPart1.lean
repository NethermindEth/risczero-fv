...
def part1_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%5" }] ←
                (Option.get! (State.props st { name := "%2" }) ∧
                  Option.get! (State.felts st { name := "%4" }) = 0))[felts][{ name := "%1" }] ←
              1)["%6"] ←ₛ
...
def part1_state_update (st: State): State :=
  Γ (part1_drops (part1_state st)) ⟦Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩⟧

-- Prove that substituting part1_state for Code.part1 produces the same result
lemma part1_wp {st : State} :
  Code.getReturn (MLIR.runProgram (Code.part1;dropfelt ⟨"%4"⟩;Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩) st) ↔
  Code.getReturn (part1_state_update st) := by
  unfold MLIR.runProgram; simp only
  generalize eq : (dropfelt ⟨"%4"⟩;Code.part2;dropfelt ⟨"%2"⟩;dropfelt ⟨"%5"⟩;dropfelt ⟨"%1"⟩;dropfelt ⟨"%6"⟩;dropfelt ⟨"%7"⟩;dropfelt ⟨"%8"⟩;dropfelt ⟨"%9"⟩;dropfelt ⟨"%10"⟩;dropfelt ⟨"%11"⟩) = prog
  unfold Code.part1
  MLIR
  rewrite [←eq]
