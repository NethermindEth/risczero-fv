...
def part13_state (st: State) : State :=
          ...
          (State.set!
            ((State.set! st { name := "data" } 10
                  (Option.get! st.felts[({ name := "%158" }: FeltVar)]!)[felts][{ name := "%159" }] ←
                Option.get! (State.felts st { name := "%20" }) -
                  Option.get! (State.felts st { name := "%8" }))[felts][{ name := "%160" }] ←
              if Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%8" }) = 0 then
                1
              else 0)
            { name := "data" } 11
            (if Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%8" }) = 0 then
...
lemma part13_updates_opaque {st : State} : 
  ...
  Code.getReturn (part13_state_update (part12_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
