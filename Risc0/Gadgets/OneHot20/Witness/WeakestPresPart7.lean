...
def part7_state (st: State) : State :=
          ...
          (State.set!
            ((State.set! st { name := "data" } 2
                  (Option.get! st.felts[({ name := "%142" }: FeltVar)]!)[felts][{ name := "%143" }] ←
            ...
            { name := "data" } 3
            (if Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%16" }) = 0 then
              1
            else
              0)) 
...
lemma part7_updates_opaque {st : State} : 
  ...
  Code.getReturn (part7_state_update (part6_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
