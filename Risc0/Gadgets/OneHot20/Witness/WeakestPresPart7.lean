          ...
          ((State.set!
              ((st[felts][{ name := "%151" }] ←
                  Option.get! (State.felts st { name := "%20" }) -
                    Option.get! (State.felts st { name := "%12" }))[felts][{ name := "%152" }] ←
                if
                    Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%12" }) =
                      0 then
                  1
                else 0)
              { name := "data" } 7
  ...
  Code.getReturn
        (part7_state_update
          ({
              buffers :=
                (Map.empty[{ name := "code" }] ←ₘ [[some x0]])[{ name := "data" }] ←ₘ
                  [[some (if x0 = 0 then 1 else 0), some (if x0 - 1 = 0 then 1 else 0),
                      ...
                      some (if x0 - 6 = 0 then 1 else 0), none, none, none, none, none, none, none, none, none, none,
                      none, none, none]],
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%12" }] ←
            7)) =
