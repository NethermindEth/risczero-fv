...
def part28_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%54" : FeltVar }] ←
                (st.felts { name := "%51" : FeltVar }).get! + (st.felts { name := "%53" : FeltVar }).get!)["%55"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (13 : ℕ))[felts][{ name := "%56" : FeltVar }] ←
            (((st[felts][{ name := "%54" : FeltVar }] ←
                        (st.felts { name := "%51" : FeltVar }).get! +
                          (st.felts { name := "%53" : FeltVar }).get!)["%55"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (13 : ℕ)).felts
                  { name := "%55" : FeltVar }).get! *
              (st.felts { name := "%6" : FeltVar }).get!)[felts][{ name := "%57" : FeltVar }] ←
          (st.felts { name := "%51" : FeltVar }).get! + (st.felts { name := "%53" : FeltVar }).get! +
            (((st[felts][{ name := "%54" : FeltVar }] ←
                        (st.felts { name := "%51" : FeltVar }).get! +
                          (st.felts { name := "%53" : FeltVar }).get!)["%55"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (13 : ℕ)).felts
