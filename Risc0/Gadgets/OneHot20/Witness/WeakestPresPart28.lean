...
def part28_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%61" }] ←
              ...
              getImpl st { name := "data" } 0 14)[felts][{ name := "%64" }] ←
            Option.get!
                (State.felts
                  (((st[felts][{ name := "%61" }] ←
                        Option.get! (State.felts st { name := "%60" }) +
  ...
  Code.getReturn
        (part28_state_update
          ((({
                ...
                feltBitAnd x1 12 * 1509949441 * 4)[felts][{ name := "%60" }] ←
              feltBitAnd x1 64 * 1981808641 * 4)[felts][{ name := "%58" }] ←
            feltBitAnd x1 48 * 1887436801)) =
