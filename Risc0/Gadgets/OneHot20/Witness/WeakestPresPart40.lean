...
def part40_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%34" : FeltVar }).get! *
              ((st.felts { name := "%18" : FeltVar }).get! - (st.felts { name := "%34" : FeltVar }).get!))
            ((st[felts][{ name := "%95" : FeltVar }] ←
                (st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%34" : FeltVar }).get!)[felts][{ name := "%96" : FeltVar }] ←
              (st.felts { name := "%34" : FeltVar }).get! *
                ((st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%34" : FeltVar }).get!)))[felts][{ name := "%97" : FeltVar }] ←
          (st.felts { name := "%94" : FeltVar }).get! + (st.felts { name := "%34" : FeltVar }).get!) 
...
lemma part40_updates_opaque {st : State} : 
  ...
  try simp [part39_state_update, part40_wp]
