...
def part15_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%167" : FeltVar }] ←
                  (st.felts { name := "%20" : FeltVar }).get! -
                    (st.felts { name := "%4" : FeltVar }).get!)[felts][{ name := "%168" : FeltVar }] ←
                if
                    (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%4" : FeltVar }).get! =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt)).set!
...
lemma part15_updates_opaque {st : State} : 
  Code.getReturn (part14_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part15_state_update (part14_drops (part14_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part14_state_update, part15_wp]

lemma part15_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
  ...
  Code.getReturn
      (part15_state_update
        ({
            buffers :=
              (Map.empty[{ name := "code" : BufferVar }] ←ₘ [[some code0]])[{ name := "data" : BufferVar }] ←ₘ
                [[some (if code0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                    ...
                    some (if code0 - (14 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none, none,
                    none]],
            bufferWidths :=
            ...
            vars :=
              [{ name := "code" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%4" : FeltVar }] ←
          (15 : Felt)))
