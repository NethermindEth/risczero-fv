...
def part3_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%12" }] ← 7)[felts][{ name := "%13" }] ← 6)[felts][{ name := "%14" }] ←
              5)[felts][{ name := "%15" }] ←
            4) 
...
lemma part3_updates_opaque {st : State} : 
  ...
  Code.getReturn (part3_state_update (part2_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
