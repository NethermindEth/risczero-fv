...
def part38_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%28" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%28" })))
              ((st[felts][{ name := "%89" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%28" }))[felts][{ name := "%90" }] ←
                Option.get! (State.felts st { name := "%28" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%28" }))))[felts][{ name := "%91" }] ←
            Option.get! (State.felts st { name := "%88" }) + Option.get! (State.felts st { name := "%28" })) 
...
lemma part38_updates_opaque {st : State} : 
  ...
  Code.getReturn (part38_state_update (part37_drops (part37_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part37_state_update, part38_wp]

lemma part38_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part38_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                ((((((((((((((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%28" }] ←ₘ
                                                if x0 - (4 : Felt) = (0 : Felt) then (1 : Felt)
                                                else (0 : Felt))[{ name := "%31" }] ←ₘ
                                              if x0 - (5 : Felt) = (0 : Felt) then (1 : Felt)
                                              else (0 : Felt))[{ name := "%34" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%88" }] ←
