...
def part38_state (st: State) : State :=
        ...
        ((withEqZero
            ...
            ((st[felts][{ name := "%89" : FeltVar }] ←
                Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%28" : FeltVar }))[felts][{ name := "%90" : FeltVar }] ←
              Option.get! (State.felts st { name := "%28" : FeltVar }) *
                (Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%28" : FeltVar }))))[felts][{ name := "%91" : FeltVar }] ←
          Option.get! (State.felts st { name := "%88" : FeltVar }) +
            Option.get! (State.felts st { name := "%28" : FeltVar })) 
