...
def part42_state (st: State) : State :=
        ...
        ((withEqZero
            ...
            ((st[felts][{ name := "%101" : FeltVar }] ←
                Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%40" : FeltVar }))[felts][{ name := "%102" : FeltVar }] ←
              Option.get! (State.felts st { name := "%40" : FeltVar }) *
                (Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%40" : FeltVar }))))[felts][{ name := "%103" : FeltVar }] ←
          Option.get! (State.felts st { name := "%100" : FeltVar }) +
            Option.get! (State.felts st { name := "%40" : FeltVar })) 
