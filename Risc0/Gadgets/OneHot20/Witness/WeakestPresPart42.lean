...
lemma part42_updates_opaque {st : State} : 
  ...
  Code.getReturn (part42_state_update (part41_drops (part41_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part41_state_update, part42_wp]

lemma part42_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part42_state_update
          ({
              ...
              vars :=
                [{ name := "code" : BufferVar },
                  { name := "data" : BufferVar }] }[felts][{ name := "%100" : FeltVar }] ←
