...
def part42_state (st: State) : State :=
          ...
          ((withEqZero
              ...
              ((st[felts][{ name := "%101" }] ←
                ...
                Option.get! (State.felts st { name := "%40" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%40" }))))[felts][{ name := "%103" }] ←
            Option.get! (State.felts st { name := "%100" }) + Option.get! (State.felts st { name := "%40" })) 
  ...
  Code.getReturn
        (part42_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                ((((((((((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%40" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%100" }] ←
