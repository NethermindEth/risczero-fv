...
def part49_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%61" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%61" })))
              ((st[felts][{ name := "%122" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%61" }))[felts][{ name := "%123" }] ←
                Option.get! (State.felts st { name := "%61" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%61" }))))[felts][{ name := "%124" }] ←
            Option.get! (State.felts st { name := "%121" }) + Option.get! (State.felts st { name := "%61" })) 
...
lemma part49_updates_opaque {st : State} : 
  ...
  Code.getReturn (part49_state_update (part48_drops (part48_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part48_state_update, part49_wp]

lemma part49_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part49_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                (((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%61" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%121" }] ←
