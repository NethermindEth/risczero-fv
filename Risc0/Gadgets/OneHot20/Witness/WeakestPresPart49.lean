...
def part49_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%61" : FeltVar }).get! *
              ((st.felts { name := "%18" : FeltVar }).get! - (st.felts { name := "%61" : FeltVar }).get!))
            ((st[felts][{ name := "%122" : FeltVar }] ←
                (st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%61" : FeltVar }).get!)[felts][{ name := "%123" : FeltVar }] ←
              (st.felts { name := "%61" : FeltVar }).get! *
                ((st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%61" : FeltVar }).get!)))[felts][{ name := "%124" : FeltVar }] ←
          (st.felts { name := "%121" : FeltVar }).get! + (st.felts { name := "%61" : FeltVar }).get!) 
...
lemma part49_updates_opaque {st : State} : 
  ...
  try simp [part48_state_update, part49_wp]
