...
def part44_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%46" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%46" })))
              ((st[felts][{ name := "%107" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%46" }))[felts][{ name := "%108" }] ←
                Option.get! (State.felts st { name := "%46" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%46" }))))[felts][{ name := "%109" }] ←
            Option.get! (State.felts st { name := "%106" }) + Option.get! (State.felts st { name := "%46" })) 
...
lemma part44_updates_opaque {st : State} : 
  ...
  Code.getReturn (part44_state_update (part43_drops (part43_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part43_state_update, part44_wp]

lemma part44_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part44_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                ((((((((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%46" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%106" }] ←
