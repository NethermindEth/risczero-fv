...
def part19_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%175" : FeltVar }] ←
                  (st.felts { name := "%20" : FeltVar }).get! -
                    (st.felts { name := "%0" : FeltVar }).get!)[felts][{ name := "%176" : FeltVar }] ←
                if
                    (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%0" : FeltVar }).get! =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt)).set!
            { name := "data" : BufferVar } (19 : ℕ)
            (if
                (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%0" : FeltVar }).get! =
                  (0 : Felt) then
              (1 : Felt)
...
lemma part19_updates_opaque {st : State} : 
  Code.getReturn (part18_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part19_state_update (part18_drops (part18_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part18_state_update, part19_wp]

lemma part19_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
            ...
            buffers :=
              (Map.empty[{ name := "code" : BufferVar }] ←ₘ [[some code0]])[{ name := "data" : BufferVar }] ←ₘ
                [[some (if code0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                    ...
                    some (if code0 - (18 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none]],
            bufferWidths :=
