...
lemma part8_updates_opaque {st : State} : 
  ...
  Code.getReturn (part8_state_update (part7_drops (part7_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part7_state_update, part8_wp]

lemma part8_cumulative_wp {x0: Felt} :
              ...
              buffers :=
                (Map.empty[{ name := "code" : BufferVar }] ←ₘ [[some x0]])[{ name := "data" : BufferVar }] ←ₘ
                  [[some (if x0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                      ...
                      some (if x0 - (7 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none, none,
