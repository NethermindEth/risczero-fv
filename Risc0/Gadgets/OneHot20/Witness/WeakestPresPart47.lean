...
def part47_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%55" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%55" })))
              ((st[felts][{ name := "%116" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%55" }))[felts][{ name := "%117" }] ←
                Option.get! (State.felts st { name := "%55" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%55" }))))[felts][{ name := "%118" }] ←
            Option.get! (State.felts st { name := "%115" }) + Option.get! (State.felts st { name := "%55" })) 
...
lemma part47_updates_opaque {st : State} : 
  ...
  Code.getReturn (part47_state_update (part46_drops (part46_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part46_state_update, part47_wp]

lemma part47_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part47_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                (((((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%55" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%115" }] ←
