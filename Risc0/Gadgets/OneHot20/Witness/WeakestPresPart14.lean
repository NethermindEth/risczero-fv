          ...
          ((State.set!
              ((st[felts][{ name := "%165" }] ←
                  Option.get! (State.felts st { name := "%20" }) -
                    Option.get! (State.felts st { name := "%5" }))[felts][{ name := "%166" }] ←
                if
                    Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%5" }) =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt))
              { name := "data" } (14 : ℕ)
...
lemma part14_updates_opaque {st : State} : 
  ...
  Code.getReturn (part14_state_update (part13_drops (part13_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part13_state_update, part14_wp]

lemma part14_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part14_state_update
          ({
              buffers :=
                (Map.empty[{ name := "code" }] ←ₘ [[some x0]])[{ name := "data" }] ←ₘ
                  [[some (if x0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                      ...
                      some (if x0 - (13 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none, none,
                      none, none]],
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%5" }] ←
            (14 : Felt))) =
