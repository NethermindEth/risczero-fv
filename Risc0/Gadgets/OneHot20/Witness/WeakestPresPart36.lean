...
def part36_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%22" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%22" })))
              ((st[felts][{ name := "%83" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%22" }))[felts][{ name := "%84" }] ←
                Option.get! (State.felts st { name := "%22" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%22" }))))[felts][{ name := "%85" }] ←
...
lemma part36_updates_opaque {st : State} : 
  ...
  Code.getReturn (part36_state_update (part35_drops (part35_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part35_state_update, part36_wp]

lemma part36_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part36_state_update
          ({
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%82" }] ←
