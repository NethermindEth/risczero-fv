...
def part36_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%22" : FeltVar }).get! *
              ((st.felts { name := "%18" : FeltVar }).get! - (st.felts { name := "%22" : FeltVar }).get!))
            ((st[felts][{ name := "%83" : FeltVar }] ←
                (st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%22" : FeltVar }).get!)[felts][{ name := "%84" : FeltVar }] ←
              (st.felts { name := "%22" : FeltVar }).get! *
                ((st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%22" : FeltVar }).get!)))[felts][{ name := "%85" : FeltVar }] ←
          (st.felts { name := "%82" : FeltVar }).get! + (st.felts { name := "%22" : FeltVar }).get!) 
