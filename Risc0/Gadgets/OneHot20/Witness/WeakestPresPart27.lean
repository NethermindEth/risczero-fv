...
def part27_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%50" }] ←
                  Option.get! (State.felts st { name := "%49" }) *
                    Option.get! (State.felts st { name := "%8" }))[felts][{ name := "%51" }] ←
                Option.get! (State.felts st { name := "%48" }) +
                  Option.get! (State.felts st { name := "%49" }) *
                    Option.get! (State.felts st { name := "%8" }))["%52"] ←ₛ
              getImpl st { name := "data" } 0 12)[felts][{ name := "%53" }] ←
            Option.get!
                (State.felts
                  (((st[felts][{ name := "%50" }] ←
                        Option.get! (State.felts st { name := "%49" }) *
  ...
  Code.getReturn
        (part27_state_update
          (((((((((((({
                ...
                if x0 - 10 = 0 then 1 else 0)[felts][{ name := "%48" }] ←
              (if x0 - 1 = 0 then 1 else 0) + (if x0 - 2 = 0 then 1 else 0) * 2 + (if x0 - 3 = 0 then 1 else 0) * 3 +
                ...
                (if x0 - 10 = 0 then 1 else 0) * 10)[felts][{ name := "%49" }] ←
            if x0 - 11 = 0 then 1 else 0)) =
