...
def part10_state (st: State) : State :=
          ...
          (State.set!
            ((State.set! st { name := "data" } 6
                  (Option.get! st.felts[({ name := "%150" }: FeltVar)]!)[felts][{ name := "%151" }] ←
            ...
            { name := "data" } 7
            (if Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%12" }) = 0 then
              1
            else
              0)) 
...
lemma part10_updates_opaque {st : State} : 
  ...
  Code.getReturn (part10_state_update (part9_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
