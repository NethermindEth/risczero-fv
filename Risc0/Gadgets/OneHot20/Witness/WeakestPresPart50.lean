...
def part50_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%64" : FeltVar }).get! *
              ((st.felts { name := "%18" : FeltVar }).get! - (st.felts { name := "%64" : FeltVar }).get!))
            ((st[felts][{ name := "%125" : FeltVar }] ←
                (st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%64" : FeltVar }).get!)[felts][{ name := "%126" : FeltVar }] ←
              (st.felts { name := "%64" : FeltVar }).get! *
                ((st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%64" : FeltVar }).get!)))[felts][{ name := "%127" : FeltVar }] ←
          (st.felts { name := "%124" : FeltVar }).get! + (st.felts { name := "%64" : FeltVar }).get!) 
...
lemma part50_updates_opaque {st : State} : 
  ...
  try simp [part49_state_update, part50_wp]
