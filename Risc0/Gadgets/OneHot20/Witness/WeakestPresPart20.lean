...
def part20_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%23" }] ←
                  Option.get! (State.felts st { name := "%22" }) *
                    Option.get! (State.felts st { name := "%17" }))["%21"] ←ₛ
                getImpl st { name := "data" } (0 : Back) (1 : ℕ))[felts][{ name := "%24" }] ←
              Option.get!
                  (State.felts
                    ((st[felts][{ name := "%23" }] ←
                        Option.get! (State.felts st { name := "%22" }) *
                          Option.get! (State.felts st { name := "%17" }))["%21"] ←ₛ
                      getImpl st { name := "data" } (0 : Back) (1 : ℕ))
                    { name := "%21" }) +
                Option.get! (State.felts st { name := "%22" }) *
                  Option.get! (State.felts st { name := "%17" }))["%25"] ←ₛ
            getImpl
              (st[felts][{ name := "%23" }] ←
                Option.get! (State.felts st { name := "%22" }) * Option.get! (State.felts st { name := "%17" }))
              { name := "data" } (0 : Back) (3 : ℕ)) 
...
lemma part20_updates_opaque {st : State} : 
  ...
  Code.getReturn (part20_state_update (part19_drops (part19_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
