...
def part20_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%23" : FeltVar }] ←
                (st.felts { name := "%22" : FeltVar }).get! * (st.felts { name := "%17" : FeltVar }).get!)["%21"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))[felts][{ name := "%24" : FeltVar }] ←
            (((st[felts][{ name := "%23" : FeltVar }] ←
                        (st.felts { name := "%22" : FeltVar }).get! *
                          (st.felts { name := "%17" : FeltVar }).get!)["%21"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ)).felts
                  { name := "%21" : FeltVar }).get! +
              (st.felts { name := "%22" : FeltVar }).get! * (st.felts { name := "%17" : FeltVar }).get!)["%25"] ←ₛ
...
lemma part20_updates_opaque {st : State} : 
  Code.getReturn (part19_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part20_state_update (part19_drops (part19_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part19_state_update, part20_wp]

lemma part20_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
  ...
  Code.getReturn
      (part20_state_update
        ({
            ...
            vars :=
              [{ name := "code" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%22" : FeltVar }] ←
