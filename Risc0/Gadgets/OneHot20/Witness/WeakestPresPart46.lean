...
def part46_state (st: State) : State :=
          ...
          ((withEqZero
              (Option.get! (State.felts st { name := "%52" }) *
                (Option.get! (State.felts st { name := "%18" }) - Option.get! (State.felts st { name := "%52" })))
              ((st[felts][{ name := "%113" }] ←
                  Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%52" }))[felts][{ name := "%114" }] ←
                Option.get! (State.felts st { name := "%52" }) *
                  (Option.get! (State.felts st { name := "%18" }) -
                    Option.get! (State.felts st { name := "%52" }))))[felts][{ name := "%115" }] ←
            Option.get! (State.felts st { name := "%112" }) + Option.get! (State.felts st { name := "%52" })) 
  ...
  Code.getReturn
        (part46_state_update
          ({
              ...
              cycle := (0 : ℕ),
              felts :=
                ((((((((Map.empty[{ name := "%18" }] ←ₘ (1 : Felt))[{ name := "%52" }] ←ₘ
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%112" }] ←
