...
def part46_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%52" : FeltVar }).get! *
              ((st.felts { name := "%18" : FeltVar }).get! - (st.felts { name := "%52" : FeltVar }).get!))
            ((st[felts][{ name := "%113" : FeltVar }] ←
                (st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%52" : FeltVar }).get!)[felts][{ name := "%114" : FeltVar }] ←
              (st.felts { name := "%52" : FeltVar }).get! *
                ((st.felts { name := "%18" : FeltVar }).get! -
                  (st.felts { name := "%52" : FeltVar }).get!)))[felts][{ name := "%115" : FeltVar }] ←
          (st.felts { name := "%112" : FeltVar }).get! + (st.felts { name := "%52" : FeltVar }).get!) 
...
lemma part46_updates_opaque {st : State} : 
  ...
  try simp [part45_state_update, part46_wp]
