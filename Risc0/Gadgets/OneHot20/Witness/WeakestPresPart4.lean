...
def part4_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%145" : FeltVar }] ←
                  (st.felts { name := "%20" : FeltVar }).get! -
                    (st.felts { name := "%15" : FeltVar }).get!)[felts][{ name := "%146" : FeltVar }] ←
                if
                    (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%15" : FeltVar }).get! =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt)).set!
            { name := "data" : BufferVar } (4 : ℕ)
            (if
                (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%15" : FeltVar }).get! =
                  (0 : Felt) then
              (1 : Felt)
            else (0 : Felt)))[felts][{ name := "%14" : FeltVar }] ←
          (5 : Felt)) 
...
lemma part4_updates_opaque {st : State} : 
  Code.getReturn (part3_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part4_state_update (part3_drops (part3_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part3_state_update, part4_wp]

lemma part4_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
  ...
  Code.getReturn
      (part4_state_update
        ({
            buffers :=
              (Map.empty[{ name := "code" : BufferVar }] ←ₘ [[some code0]])[{ name := "data" : BufferVar }] ←ₘ
                [[some (if code0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                    ...
                    some (if code0 - (3 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none, none,
            ...
            vars :=
              [{ name := "code" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%15" : FeltVar }] ←
          (4 : Felt)))
