...
def part4_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%16" }] ← 3)[felts][{ name := "%17" }] ← 2)[felts][{ name := "%18" }] ←
              1)[felts][{ name := "%19" }] ←
            0) 
...
lemma part4_updates_opaque {st : State} : 
  ...
  Code.getReturn (part4_state_update (part3_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
