...
def part4_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%13" }] ← 8)[felts][{ name := "%80" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%23" })) 8)[felts][{ name := "%12" }] ←
              1761607681)[felts][{ name := "%81" }] ←
            feltBitAnd (Option.get! (State.felts st { name := "%23" })) 8 * 1761607681) 
...
lemma part4_updates_opaque {st : State} : 
  ...
  Code.getReturn (part4_state_update (part3_drops (part3_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part3_state_update, part4_wp]

lemma part4_cumulative_wp {x0 x1 x2 x3: Felt} :
        ...
        (part4_state_update
          {
            buffers :=
              (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                [[none, some (feltBitAnd x3 96 * 1950351361), none, none, none, none, none, none, none,
                    some (feltBitAnd x3 16 * 1887436801), some (feltBitAnd x3 128 * 1997537281), none, none, none, none,
                    none, none, none]],
