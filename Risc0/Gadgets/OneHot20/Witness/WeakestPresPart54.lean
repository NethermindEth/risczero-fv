...
def part54_state (st: State) : State :=
          ...
          (withEqZero (Option.get! (State.felts st { name := "%136" }) - Option.get! (State.felts st { name := "%18" }))
            ((st[felts][{ name := "%137" }] ←
                Option.get! (State.felts st { name := "%136" }) -
                  Option.get! (State.felts st { name := "%18" }))[felts][{ name := "%19" }] ←
              (0 : Felt))) 
              ...
              constraints :=
                [(if x0 - (19 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)) = (0 : Felt) ∨
                  ...
                  (if x0 - (1 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)) +
                      ...
                      x0 =
                    (0 : Felt)],
              cycle := (0 : ℕ), felts := Map.empty[{ name := "%18" }] ←ₘ (1 : Felt), isFailed := false,
              props := Map.empty, vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%136" }] ←
