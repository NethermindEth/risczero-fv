...
def part22_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%30" }] ←
                  Option.get! (State.felts st { name := "%27" }) +
                    Option.get! (State.felts st { name := "%29" }))["%31"] ←ₛ
                getImpl st { name := "data" } (0 : Back) (5 : ℕ))[felts][{ name := "%32" }] ←
              Option.get!
                  (State.felts
                    ((st[felts][{ name := "%30" }] ←
                        Option.get! (State.felts st { name := "%27" }) +
                          Option.get! (State.felts st { name := "%29" }))["%31"] ←ₛ
                      getImpl st { name := "data" } (0 : Back) (5 : ℕ))
                    { name := "%31" }) *
                Option.get! (State.felts st { name := "%14" }))[felts][{ name := "%33" }] ←
            Option.get! (State.felts st { name := "%27" }) + Option.get! (State.felts st { name := "%29" }) +
              Option.get!
                  (State.felts
                    ((st[felts][{ name := "%30" }] ←
                        Option.get! (State.felts st { name := "%27" }) +
...
lemma part22_updates_opaque {st : State} : 
  ...
  Code.getReturn (part22_state_update (part21_drops (part21_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part21_state_update, part22_wp]

lemma part22_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part22_state_update
          (((((({
                  ...
                  if x0 - (3 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt))[felts][{ name := "%27" }] ←
                (if x0 - (1 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)) +
                  ...
                  (if x0 - (3 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)) *
                    (3 : Felt))[felts][{ name := "%28" }] ←
              if x0 - (4 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt))[felts][{ name := "%29" }] ←
