...
def part22_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%30" : FeltVar }] ←
                (st.felts { name := "%27" : FeltVar }).get! + (st.felts { name := "%29" : FeltVar }).get!)["%31"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (5 : ℕ))[felts][{ name := "%32" : FeltVar }] ←
            (((st[felts][{ name := "%30" : FeltVar }] ←
                        (st.felts { name := "%27" : FeltVar }).get! +
                          (st.felts { name := "%29" : FeltVar }).get!)["%31"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (5 : ℕ)).felts
                  { name := "%31" : FeltVar }).get! *
              (st.felts { name := "%14" : FeltVar }).get!)[felts][{ name := "%33" : FeltVar }] ←
          (st.felts { name := "%27" : FeltVar }).get! + (st.felts { name := "%29" : FeltVar }).get! +
            (((st[felts][{ name := "%30" : FeltVar }] ←
                        (st.felts { name := "%27" : FeltVar }).get! +
                          (st.felts { name := "%29" : FeltVar }).get!)["%31"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (5 : ℕ)).felts
...
lemma part22_updates_opaque {st : State} : 
  Code.getReturn (part21_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part22_state_update (part21_drops (part21_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part21_state_update, part22_wp]

lemma part22_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
  ...
  Code.getReturn
      (part22_state_update
        (((((({
                      ...
                      bufferWidths :=
                        (Map.empty[{ name := "data" : BufferVar }] ←ₘ (20 : ℕ))[{ name := "code" : BufferVar }] ←ₘ
                          (1 : ℕ),
                      cycle := (0 : ℕ),
                      felts :=
                        ((((((((((((((((Map.empty[{ name := "%20" : FeltVar }] ←ₘ code0)[{ name := "%18" : FeltVar }] ←ₘ
                                                        (1 : Felt))[{ name := "%14" : FeltVar }] ←ₘ
                ...
                else (0 : Felt))[felts][{ name := "%27" : FeltVar }] ←
            ...
            if code0 - (4 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt))[felts][{ name := "%29" : FeltVar }] ←
