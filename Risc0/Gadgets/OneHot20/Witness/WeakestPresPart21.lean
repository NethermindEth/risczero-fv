            ...
            Option.get!
                (State.felts
                  (((st[felts][{ name := "%26" : FeltVar }] ←
                        Option.get! (State.felts st { name := "%25" : FeltVar }) *
                          Option.get!
                            (State.felts st { name := "%16" : FeltVar }))[felts][{ name := "%27" : FeltVar }] ←
                      Option.get! (State.felts st { name := "%24" : FeltVar }) +
                        Option.get! (State.felts st { name := "%25" : FeltVar }) *
                          Option.get! (State.felts st { name := "%16" : FeltVar }))["%28"] ←ₛ
  ...
  Code.getReturn
        (part21_state_update
          (((({
                  ...
                  if x0 - (2 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt))[felts][{ name := "%21" : FeltVar }] ←
                if x0 - (1 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt))[felts][{ name := "%24" : FeltVar }] ←
