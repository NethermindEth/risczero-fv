...
def part21_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%26" : FeltVar }] ←
                (st.felts { name := "%25" : FeltVar }).get! *
                  (st.felts { name := "%16" : FeltVar }).get!)[felts][{ name := "%27" : FeltVar }] ←
              (st.felts { name := "%24" : FeltVar }).get! +
                (st.felts { name := "%25" : FeltVar }).get! * (st.felts { name := "%16" : FeltVar }).get!)["%28"] ←ₛ
          ...
          ((((st[felts][{ name := "%26" : FeltVar }] ←
                      ...
                      (st.felts { name := "%24" : FeltVar }).get! +
                        (st.felts { name := "%25" : FeltVar }).get! *
                          (st.felts { name := "%16" : FeltVar }).get!)["%28"] ←ₛ
                    getImpl st { name := "data" : BufferVar } (0 : Back) (4 : ℕ)).felts
                { name := "%28" : FeltVar }).get! *
      ...
      (part21_state_update
        (((({
            ...
            (if code0 - (1 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)) +
              if code0 - (2 : Felt) = (0 : Felt) then (2 : Felt) else (0 : Felt))[felts][{ name := "%25" : FeltVar }] ←
