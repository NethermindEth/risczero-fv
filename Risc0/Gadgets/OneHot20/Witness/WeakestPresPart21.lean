...
def part21_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%26" }] ←
              ...
              getImpl st { name := "data" } 0 4)[felts][{ name := "%29" }] ←
            Option.get!
                (State.felts
                  (((st[felts][{ name := "%26" }] ←
                        Option.get! (State.felts st { name := "%25" }) *
...
lemma part21_updates_opaque {st : State} : 
  ...
  Code.getReturn (part21_state_update (part20_drops (part20_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part20_state_update, part21_wp]

lemma part21_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part21_state_update
          (((({
                ...
                if x0 - 1 = 0 then 1 else 0)[felts][{ name := "%24" }] ←
              (if x0 - 1 = 0 then 1 else 0) + (if x0 - 2 = 0 then 1 else 0) * 2)[felts][{ name := "%25" }] ←
            if x0 - 3 = 0 then 1 else 0)) =
