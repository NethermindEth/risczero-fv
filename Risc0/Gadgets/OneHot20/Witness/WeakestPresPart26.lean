...
def part26_state (st: State) : State :=
        ...
        ((((st["%46"] ←ₛ
              ...
              Option.get!
                  (State.felts (st["%46"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ))
                    { name := "%46" : FeltVar }) *
                Option.get! (State.felts st { name := "%9" : FeltVar }))[felts][{ name := "%48" : FeltVar }] ←
            Option.get! (State.felts st { name := "%45" : FeltVar }) +
              Option.get!
                  (State.felts (st["%46"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ))
                    { name := "%46" : FeltVar }) *
                Option.get! (State.felts st { name := "%9" : FeltVar }))["%49"] ←ₛ
