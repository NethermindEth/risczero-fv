          ...
          ((State.set!
              ((st[felts][{ name := "%155" }] ←
                  Option.get! (State.felts st { name := "%20" }) -
                    Option.get! (State.felts st { name := "%10" }))[felts][{ name := "%156" }] ←
                if
                    Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%10" }) =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt))
              { name := "data" } (9 : ℕ)
  ...
  Code.getReturn
        (part9_state_update
          ({
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%10" }] ←
            (9 : Felt))) =
