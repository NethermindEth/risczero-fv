          ...
          ((State.set!
              ((st[felts][{ name := "%171" }] ←
                  Option.get! (State.felts st { name := "%20" }) -
                    Option.get! (State.felts st { name := "%2" }))[felts][{ name := "%172" }] ←
                if
                    Option.get! (State.felts st { name := "%20" }) - Option.get! (State.felts st { name := "%2" }) =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt))
              { name := "data" } (17 : ℕ)
...
lemma part17_updates_opaque {st : State} : 
  ...
  Code.getReturn (part17_state_update (part16_drops (part16_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17, y18, y19] := by
  simp [part16_state_update, part17_wp]

lemma part17_cumulative_wp {x0: Felt} :
  ...
  Code.getReturn
        (part17_state_update
          ({
              buffers :=
                (Map.empty[{ name := "code" }] ←ₘ [[some x0]])[{ name := "data" }] ←ₘ
                  [[some (if x0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                      ...
                      some (if x0 - (16 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none]],
              ...
              vars := [{ name := "code" }, { name := "data" }] }[felts][{ name := "%2" }] ←
            (17 : Felt))) =
