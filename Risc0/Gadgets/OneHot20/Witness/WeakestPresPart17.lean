...
def part17_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%171" : FeltVar }] ←
                  (st.felts { name := "%20" : FeltVar }).get! -
                    (st.felts { name := "%2" : FeltVar }).get!)[felts][{ name := "%172" : FeltVar }] ←
                if
                    (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%2" : FeltVar }).get! =
                      (0 : Felt) then
                  (1 : Felt)
                else (0 : Felt)).set!
            { name := "data" : BufferVar } (17 : ℕ)
            (if
                (st.felts { name := "%20" : FeltVar }).get! - (st.felts { name := "%2" : FeltVar }).get! =
                  (0 : Felt) then
              (1 : Felt)
...
lemma part17_updates_opaque {st : State} : 
  Code.getReturn (part16_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) ↔
  Code.getReturn (part17_state_update (part16_drops (part16_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17, data18, data19]) := by
  try simp [part16_state_update, part17_wp]

lemma part17_cumulative_wp {code0: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Option Felt} :
  ...
  Code.getReturn
      (part17_state_update
        ({
            buffers :=
              (Map.empty[{ name := "code" : BufferVar }] ←ₘ [[some code0]])[{ name := "data" : BufferVar }] ←ₘ
                [[some (if code0 = (0 : Felt) then (1 : Felt) else (0 : Felt)),
                    ...
                    some (if code0 - (16 : Felt) = (0 : Felt) then (1 : Felt) else (0 : Felt)), none, none, none]],
            bufferWidths :=
            ...
            vars :=
              [{ name := "code" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%2" : FeltVar }] ←
          (17 : Felt)))
