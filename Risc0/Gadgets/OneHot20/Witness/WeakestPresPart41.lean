...
def part41_state (st: State) : State :=
        ...
        ((withEqZero
            ...
            ((st[felts][{ name := "%98" : FeltVar }] ←
                Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%37" : FeltVar }))[felts][{ name := "%99" : FeltVar }] ←
              Option.get! (State.felts st { name := "%37" : FeltVar }) *
                (Option.get! (State.felts st { name := "%18" : FeltVar }) -
                  Option.get! (State.felts st { name := "%37" : FeltVar }))))[felts][{ name := "%100" : FeltVar }] ←
          Option.get! (State.felts st { name := "%97" : FeltVar }) +
            Option.get! (State.felts st { name := "%37" : FeltVar })) 
