...
def part35_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%140" : PropVar }] ←
                ((st.props { name := "%136" : PropVar }).get! ∧
                  (st.felts { name := "%139" : FeltVar }).get! = (0 : Felt)))[felts][{ name := "%141" : FeltVar }] ←
              (st.felts { name := "%137" : FeltVar }).get! +
                (st.felts { name := "%61" : FeltVar }).get!)[felts][{ name := "%142" : FeltVar }] ←
            (st.felts { name := "%0" : FeltVar }).get! -
              (st.felts { name := "%64" : FeltVar }).get!)[felts][{ name := "%143" : FeltVar }] ←
          (st.felts { name := "%64" : FeltVar }).get! *
            ((st.felts { name := "%0" : FeltVar }).get! - (st.felts { name := "%64" : FeltVar }).get!)) 
