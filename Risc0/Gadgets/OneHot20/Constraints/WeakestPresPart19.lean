...
def part19_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%77" }] ←
                (Option.get! (State.props st { name := "%19" }) ∧
                  Option.get! (State.felts st { name := "%76" }) = (0 : Felt)))[felts][{ name := "%0" }] ←
              (1 : Felt))["%78"] ←ₛ
            getImpl st { name := "data" } (0 : Back) (0 : ℕ))[felts][{ name := "%79" }] ←
          (1 : Felt) -
            Option.get!
              (State.felts
                (((st[props][{ name := "%77" }] ←
                      (Option.get! (State.props st { name := "%19" }) ∧
                        Option.get! (State.felts st { name := "%76" }) = (0 : Felt)))[felts][{ name := "%0" }] ←
                    (1 : Felt))["%78"] ←ₛ
                  getImpl st { name := "data" } (0 : Back) (0 : ℕ))
                { name := "%78" })) 
...
lemma part19_cumulative_wp {x0 y0 y1 y2 y3 y4 y5 y6 y7 y8 y9 y10 y11 y12 y13 y14 y15 y16 y17 y18 y19: Felt} :
