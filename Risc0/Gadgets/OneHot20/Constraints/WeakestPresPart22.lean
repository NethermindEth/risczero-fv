...
def part22_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%88" : PropVar }] ←
                (Option.get! (State.props st { name := "%84" : PropVar }) ∧
                  Option.get! (State.felts st { name := "%87" : FeltVar }) =
                    (0 : Felt)))[felts][{ name := "%89" : FeltVar }] ←
              Option.get! (State.felts st { name := "%85" : FeltVar }) +
                Option.get! (State.felts st { name := "%22" : FeltVar }))[felts][{ name := "%90" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%25" : FeltVar }))[felts][{ name := "%91" : FeltVar }] ←
