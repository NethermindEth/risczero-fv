...
def part23_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%92" }] ←
                (Option.get! (State.props st { name := "%88" }) ∧
                  Option.get! (State.felts st { name := "%91" }) = (0 : Felt)))[felts][{ name := "%93" }] ←
              Option.get! (State.felts st { name := "%89" }) +
                Option.get! (State.felts st { name := "%25" }))[felts][{ name := "%94" }] ←
            Option.get! (State.felts st { name := "%0" }) -
              Option.get! (State.felts st { name := "%28" }))[felts][{ name := "%95" }] ←
          Option.get! (State.felts st { name := "%28" }) *
            (Option.get! (State.felts st { name := "%0" }) - Option.get! (State.felts st { name := "%28" }))) 
      ...
      (part23_state_update
        ((((((((((((((((((((((((({
              ...
              (((y1 + y2 * (2 : Felt) + y3 * (3 : Felt) + y4 * (4 : Felt) + y5 * (5 : Felt) + y6 * (6 : Felt) +
                ...
                (y2 = (0 : Felt) ∨ (1 : Felt) - y2 = (0 : Felt))))[felts][{ name := "%89" }] ←
            y0 + y1 + y2)[felts][{ name := "%91" }] ←
          y3 * ((1 : Felt) - y3)))  := by
