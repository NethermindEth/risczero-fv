...
def part37_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%148" : PropVar }] ←
                ((st.props { name := "%144" : PropVar }).get! ∧
                  (st.felts { name := "%147" : FeltVar }).get! = (0 : Felt)))[felts][{ name := "%149" : FeltVar }] ←
              (st.felts { name := "%145" : FeltVar }).get! +
                (st.felts { name := "%67" : FeltVar }).get!)[felts][{ name := "%150" : FeltVar }] ←
            (st.felts { name := "%0" : FeltVar }).get! -
              (st.felts { name := "%70" : FeltVar }).get!)[felts][{ name := "%151" : FeltVar }] ←
          (st.felts { name := "%70" : FeltVar }).get! *
            ((st.felts { name := "%0" : FeltVar }).get! - (st.felts { name := "%70" : FeltVar }).get!)) 
