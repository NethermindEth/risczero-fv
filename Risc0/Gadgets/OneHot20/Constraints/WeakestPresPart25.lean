...
def part25_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%100" : PropVar }] ←
                (Option.get! (State.props st { name := "%96" : PropVar }) ∧
                  Option.get! (State.felts st { name := "%99" : FeltVar }) =
                    (0 : Felt)))[felts][{ name := "%101" : FeltVar }] ←
              Option.get! (State.felts st { name := "%97" : FeltVar }) +
                Option.get! (State.felts st { name := "%31" : FeltVar }))[felts][{ name := "%102" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%34" : FeltVar }))[felts][{ name := "%103" : FeltVar }] ←
