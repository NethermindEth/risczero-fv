...
def part34_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%136" : PropVar }] ←
                ((st.props { name := "%132" : PropVar }).get! ∧
                  (st.felts { name := "%135" : FeltVar }).get! = (0 : Felt)))[felts][{ name := "%137" : FeltVar }] ←
              (st.felts { name := "%133" : FeltVar }).get! +
                (st.felts { name := "%58" : FeltVar }).get!)[felts][{ name := "%138" : FeltVar }] ←
            (st.felts { name := "%0" : FeltVar }).get! -
              (st.felts { name := "%61" : FeltVar }).get!)[felts][{ name := "%139" : FeltVar }] ←
          (st.felts { name := "%61" : FeltVar }).get! *
            ((st.felts { name := "%0" : FeltVar }).get! - (st.felts { name := "%61" : FeltVar }).get!)) 
