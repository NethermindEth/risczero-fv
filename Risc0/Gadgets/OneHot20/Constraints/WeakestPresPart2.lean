...
def part2_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%26" : FeltVar }] ←
                (st.felts { name := "%25" : FeltVar }).get! *
                  (st.felts { name := "%2" : FeltVar }).get!)[felts][{ name := "%27" : FeltVar }] ←
              (st.felts { name := "%24" : FeltVar }).get! +
                (st.felts { name := "%25" : FeltVar }).get! *
                  (st.felts { name := "%2" : FeltVar }).get!)[felts][{ name := "%3" : FeltVar }] ←
            (4 : Felt))["%28"] ←ₛ
...
lemma part2_cumulative_wp {code0 data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Felt} :
  ...
  Code.getReturn
      (part2_state_update
        (((((({
                      buffers :=
                        (Map.empty[{ name := "data" : BufferVar }] ←ₘ
                            [[some data0, some data1, some data2, some data3, some data4, some data5, some data6,
                                ...
                                some data14, some data15, some data16, some data17, some data18,
                ...
                data1)[felts][{ name := "%24" : FeltVar }] ←
              data1 + data2 * (2 : Felt))[felts][{ name := "%2" : FeltVar }] ←
