...
def part8_state (st: State) : State :=
        ...
        ((((st["%32"] ←ₛ getImpl st { name := "data" } 0 11)[felts][{ name := "%38" }] ←
              Option.get! (State.felts st { name := "%37" }) +
                Option.get!
                  (State.felts (st["%32"] ←ₛ getImpl st { name := "data" } 0 11)
                    { name := "%32" }))[felts][{ name := "%39" }] ←
            (Option.get! (State.felts st { name := "%37" }) +
                Option.get! (State.felts (st["%32"] ←ₛ getImpl st { name := "data" } 0 11) { name := "%32" })) *
              Option.get! (State.felts st { name := "%1" }))[felts][{ name := "%40" }] ←
          (Option.get! (State.felts st { name := "%37" }) +
                Option.get! (State.felts (st["%32"] ←ₛ getImpl st { name := "data" } 0 11) { name := "%32" })) *
  ...
  Code.getReturn
      (part8_state_update
        (((((({
                  ...
                  4)[felts][{ name := "%1" }] ←
                16)[props][{ name := "%28" }] ←
              x3 - ((y10 * 64 + (y1 * 16 + y9 * 8 + y8 * 4 + y0)) * 2 + y13) = 0)[felts][{ name := "%31" }] ←
            y4 * 4)[felts][{ name := "%37" }] ←
          y12 * 8 + y2 * 2))  := by
