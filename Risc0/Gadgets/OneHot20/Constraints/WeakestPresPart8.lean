...
def part8_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%44" : FeltVar }] ←
                Option.get! (State.felts st { name := "%43" : FeltVar }) *
                  Option.get! (State.felts st { name := "%8" : FeltVar }))[felts][{ name := "%45" : FeltVar }] ←
              Option.get! (State.felts st { name := "%42" : FeltVar }) +
                Option.get! (State.felts st { name := "%43" : FeltVar }) *
                  Option.get! (State.felts st { name := "%8" : FeltVar }))[felts][{ name := "%9" : FeltVar }] ←
            (10 : Felt))["%46"] ←ₛ
          getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ)) 

def part8_drops (st: State) : State :=
  State.dropFelts (State.dropFelts (State.dropFelts (st) ⟨"%42"⟩) ⟨"%8"⟩) ⟨"%44"⟩
...
lemma part8_cumulative_wp {code0 data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Felt} :
  ...
  Code.getReturn
      (part8_state_update
        (((((((((((({
                                  buffers :=
                                    ((fun x => Map.empty x)[{ name := "data" : BufferVar }] ←ₘ
                                        [[some data0, some data1, some data2, some data3, some data4, some data5,
                                            ...
                                            some data17, some data18, some data19]])[{ name := "code" : BufferVar }] ←ₘ
                                      [[some code0]],
                                  bufferWidths :=
                                    ((fun x => Map.empty x)[{ name := "data" : BufferVar }] ←ₘ
                ...
                data8)[felts][{ name := "%42" : FeltVar }] ←
              data1 + data2 * (2 : Felt) + data3 * (3 : Felt) + data4 * (4 : Felt) + data5 * (5 : Felt) +
                    data6 * (6 : Felt) +
                  data7 * (7 : Felt) +
                data8 * (8 : Felt))[felts][{ name := "%8" : FeltVar }] ←
            (9 : Felt))[felts][{ name := "%43" : FeltVar }] ←
          data9))  := by
