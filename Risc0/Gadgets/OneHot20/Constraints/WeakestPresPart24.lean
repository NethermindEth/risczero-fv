...
def part24_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%96" : PropVar }] ←
                ((st.props { name := "%92" : PropVar }).get! ∧
                  (st.felts { name := "%95" : FeltVar }).get! = (0 : Felt)))[felts][{ name := "%97" : FeltVar }] ←
              (st.felts { name := "%93" : FeltVar }).get! +
                (st.felts { name := "%28" : FeltVar }).get!)[felts][{ name := "%98" : FeltVar }] ←
            (st.felts { name := "%0" : FeltVar }).get! -
              (st.felts { name := "%31" : FeltVar }).get!)[felts][{ name := "%99" : FeltVar }] ←
          (st.felts { name := "%31" : FeltVar }).get! *
            ((st.felts { name := "%0" : FeltVar }).get! - (st.felts { name := "%31" : FeltVar }).get!)) 
