...
def part20_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%80" }] ←
              ...
              (Option.get! (State.props st { name := "%77" }) ∧
                (Option.get! (State.felts st { name := "%78" }) = (0 : Felt) ∨
                  Option.get! (State.felts st { name := "%79" }) = (0 : Felt))))[felts][{ name := "%82" }] ←
            Option.get! (State.felts st { name := "%0" }) -
              Option.get! (State.felts st { name := "%21" }))[felts][{ name := "%83" }] ←
          Option.get! (State.felts st { name := "%21" }) *
            (Option.get! (State.felts st { name := "%0" }) - Option.get! (State.felts st { name := "%21" }))) 
