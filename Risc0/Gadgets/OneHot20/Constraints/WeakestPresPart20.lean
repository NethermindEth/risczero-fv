...
def part20_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%80" : FeltVar }] ←
                Option.get! (State.felts st { name := "%78" : FeltVar }) *
                  Option.get! (State.felts st { name := "%79" : FeltVar }))[props][{ name := "%81" : PropVar }] ←
              (Option.get! (State.props st { name := "%77" : PropVar }) ∧
                (Option.get! (State.felts st { name := "%78" : FeltVar }) = (0 : Felt) ∨
                  Option.get! (State.felts st { name := "%79" : FeltVar }) =
                    (0 : Felt))))[felts][{ name := "%82" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%21" : FeltVar }))[felts][{ name := "%83" : FeltVar }] ←
...
lemma part20_cumulative_wp {x0 y0 y1 y2 y3 y4 y5 y6 y7 y8 y9 y10 y11 y12 y13 y14 y15 y16 y17 y18 y19: Felt} :
                                                          ...
                                                          buffers :=
                                                            ((fun x => Map.empty x)[{ name := "data" : BufferVar }] ←ₘ
                                                                [[some y0, some y1, some y2, some y3, some y4, some y5,
                                                                    some y6, some y7, some y8, some y9, some y10,
                                                                    some y11, some y12, some y13, some y14, some y15,
                                                                    some y16, some y17, some y18,
