...
def part18_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%74" : FeltVar }] ←
                (st.felts { name := "%73" : FeltVar }).get! *
                  (st.felts { name := "%18" : FeltVar }).get!)[felts][{ name := "%75" : FeltVar }] ←
              (st.felts { name := "%72" : FeltVar }).get! +
                (st.felts { name := "%73" : FeltVar }).get! * (st.felts { name := "%18" : FeltVar }).get!)["%20"] ←ₛ
            getImpl st { name := "code" : BufferVar } (0 : Back) (0 : ℕ))[felts][{ name := "%76" : FeltVar }] ←
          (st.felts { name := "%72" : FeltVar }).get! +
              (st.felts { name := "%73" : FeltVar }).get! * (st.felts { name := "%18" : FeltVar }).get! -
            ((((st[felts][{ name := "%74" : FeltVar }] ←
                        (st.felts { name := "%73" : FeltVar }).get! *
                          (st.felts { name := "%18" : FeltVar }).get!)[felts][{ name := "%75" : FeltVar }] ←
                      (st.felts { name := "%72" : FeltVar }).get! +
                        (st.felts { name := "%73" : FeltVar }).get! *
                          (st.felts { name := "%18" : FeltVar }).get!)["%20"] ←ₛ
