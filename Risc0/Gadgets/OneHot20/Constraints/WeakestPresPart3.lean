  ...
  Code.getReturn
      (part3_state_update
        ((((((({
                  ...
                  y1)[felts][{ name := "%25" }] ←
                y3)[felts][{ name := "%27" }] ←
              y1 + y2 * (2 : Felt) + y3 * (3 : Felt))[felts][{ name := "%3" }] ←
            (4 : Felt))[felts][{ name := "%28" }] ←
          y4))  := by
