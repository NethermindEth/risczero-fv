...
def part1_state (st: State) : State :=
        ...
        ((((st["%21"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ))[felts][{ name := "%24" }] ←
              Option.get!
                  (State.felts (st["%21"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ)) { name := "%21" }) +
                Option.get! (State.felts st { name := "%23" }))[felts][{ name := "%2" }] ←
            (3 : Felt))["%25"] ←ₛ
  ...
  Code.getReturn
      (part1_state_update
        ((({
                ...
                vars := [{ name := "code" }, { name := "data" }] }[props][{ name := "%19" }] ←
              True)[felts][{ name := "%22" }] ←
            y2)[felts][{ name := "%23" }] ←
          y2 * (2 : Felt)))  := by
