...
def part29_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%116" : PropVar }] ←
                (Option.get! (State.props st { name := "%112" : PropVar }) ∧
                  Option.get! (State.felts st { name := "%115" : FeltVar }) =
                    (0 : Felt)))[felts][{ name := "%117" : FeltVar }] ←
              Option.get! (State.felts st { name := "%113" : FeltVar }) +
                Option.get! (State.felts st { name := "%43" : FeltVar }))[felts][{ name := "%118" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%46" : FeltVar }))[felts][{ name := "%119" : FeltVar }] ←
