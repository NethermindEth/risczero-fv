...
def part11_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%53" : FeltVar }] ←
                (st.felts { name := "%52" : FeltVar }).get! *
                  (st.felts { name := "%11" : FeltVar }).get!)[felts][{ name := "%54" : FeltVar }] ←
              (st.felts { name := "%51" : FeltVar }).get! +
                (st.felts { name := "%52" : FeltVar }).get! *
                  (st.felts { name := "%11" : FeltVar }).get!)[felts][{ name := "%12" : FeltVar }] ←
            (13 : Felt))["%55"] ←ₛ
          getImpl st { name := "data" : BufferVar } (0 : Back) (13 : ℕ)) 

def part11_drops (st: State) : State :=
  State.dropFelts (State.dropFelts (State.dropFelts (st) ⟨"%51"⟩) ⟨"%11"⟩) ⟨"%53"⟩
...
lemma part11_cumulative_wp {code0 data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 data18 data19: Felt} :
      ...
      (part11_state_update
        ((((((((((((((({
              ...
              data1 + data2 * (2 : Felt) + data3 * (3 : Felt) + data4 * (4 : Felt) + data5 * (5 : Felt) +
                    ...
                    data9 * (9 : Felt) +
                  data10 * (10 : Felt) +
                data11 * (11 : Felt))[felts][{ name := "%11" : FeltVar }] ←
            (12 : Felt))[felts][{ name := "%52" : FeltVar }] ←
          data12))  := by
