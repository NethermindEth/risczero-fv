...
def part36_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%144" : PropVar }] ←
                (Option.get! (State.props st { name := "%140" : PropVar }) ∧
                  Option.get! (State.felts st { name := "%143" : FeltVar }) =
                    (0 : Felt)))[felts][{ name := "%145" : FeltVar }] ←
              Option.get! (State.felts st { name := "%141" : FeltVar }) +
                Option.get! (State.felts st { name := "%64" : FeltVar }))[felts][{ name := "%146" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%67" : FeltVar }))[felts][{ name := "%147" : FeltVar }] ←
