...
def part26_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%104" : PropVar }] ←
                (Option.get! (State.props st { name := "%100" : PropVar }) ∧
                  Option.get! (State.felts st { name := "%103" : FeltVar }) =
                    (0 : Felt)))[felts][{ name := "%105" : FeltVar }] ←
              Option.get! (State.felts st { name := "%101" : FeltVar }) +
                Option.get! (State.felts st { name := "%34" : FeltVar }))[felts][{ name := "%106" : FeltVar }] ←
            Option.get! (State.felts st { name := "%0" : FeltVar }) -
              Option.get! (State.felts st { name := "%37" : FeltVar }))[felts][{ name := "%107" : FeltVar }] ←
