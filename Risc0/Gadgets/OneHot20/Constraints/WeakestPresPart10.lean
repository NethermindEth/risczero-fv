...
def part10_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%43" }] ←
                (Option.get! (State.props st { name := "%28" }) ∧
                  Option.get! (State.felts st { name := "%42" }) = 0))["%45"] ←ₛ
          ...
          getImpl
            (st[props][{ name := "%43" }] ←
              (Option.get! (State.props st { name := "%28" }) ∧ Option.get! (State.felts st { name := "%42" }) = 0))
            { name := "data" } 0 15) 
  ...
  Code.getReturn
      (part10_state_update
        ((((({
                ...
                4)[felts][{ name := "%1" }] ←
              16)[props][{ name := "%28" }] ←
            x3 - ((y10 * 64 + (y1 * 16 + y9 * 8 + y8 * 4 + y0)) * 2 + y13) = 0)[felts][{ name := "%42" }] ←
          x2 - ((y12 * 8 + y2 * 2 + y11) * 16 + y4 * 4 + y3)))  := by
