...
--   simp only [Op.eval_const, State.update_val]
--   MLIR_statement
--   simp only [Op.eval_const, State.update_val, Op.eval_getBuffer, ge_iff_le]
--   have h_cycle: 0 ≤ st.cycle := by simp_arith
--   unfold State.update
--   simp only [ge_iff_le, h_cycle]
...
--   st.felts.get! ⟨"output[1]"⟩ = (st.buffers.get! ⟨"output[1]"⟩ |>.get! (st.cycle, 1)) ∧
--   st.felts.get! ⟨"output[2]"⟩ = (st.buffers.get! ⟨"output[2]"⟩ |>.get! (st.cycle, 2))
...
--     (
--       (prog.run st |>.buffers.get! ⟨"output"⟩ |>.get! (st.cycle, 0)).get! = 0 ∨
--       (prog.run st |>.buffers.get! ⟨"output"⟩ |>.get! (st.cycle, 0)).get! = 1
--     ) ∧
--     (
--       (prog.run st |>.buffers.get! ⟨"output"⟩ |>.get! (st.cycle, 1)).get! = 0 ∨
--       (prog.run st |>.buffers.get! ⟨"output"⟩ |>.get! (st.cycle, 1)).get! = 1
...
lemma constraints_closed_form {input : Felt} {output: BufferAtTime} :
  ...
  unfold constraints MLIR.runProgram
  MLIR
  MLIR_states
  rfl
