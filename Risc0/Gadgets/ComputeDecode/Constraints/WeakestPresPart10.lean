...
def part10_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%43" }] ←
                (Option.get! (State.props st { name := "%28" }) ∧
                  Option.get! (State.felts st { name := "%42" }) = (0 : Felt)))["%45"] ←ₛ
              getImpl st { name := "data" } (0 : Back) (7 : ℕ))[felts][{ name := "%46" }] ←
            Option.get!
                (State.felts
                  ((st[props][{ name := "%43" }] ←
                      (Option.get! (State.props st { name := "%28" }) ∧
                  ...
                  { name := "%45" }) *
              Option.get! (State.felts st { name := "%4" }))["%48"] ←ₛ
          getImpl
            (st[props][{ name := "%43" }] ←
              (Option.get! (State.props st { name := "%28" }) ∧
                Option.get! (State.felts st { name := "%42" }) = (0 : Felt)))
            { name := "data" } (0 : Back) (15 : ℕ)) 
