...
def part4_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%23" : FeltVar }] ←
                (st.felts { name := "%22" : FeltVar }).get! *
                  (st.felts { name := "%5" : FeltVar }).get!)[felts][{ name := "%24" : FeltVar }] ←
              (st.felts { name := "%22" : FeltVar }).get! * (st.felts { name := "%5" : FeltVar }).get! +
                (st.felts { name := "%21" : FeltVar }).get!)[felts][{ name := "%3" : FeltVar }] ←
