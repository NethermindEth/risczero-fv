...
def part13_state (st: State) : State := 
        ...
        ((((st["%52"] ←ₛ getImpl st { name := "data" } 0 14)[felts][{ name := "%53" }] ←
              Option.get! (State.felts (st["%52"] ←ₛ getImpl st { name := "data" } 0 14) { name := "%52" }) *
                Option.get! (State.felts st { name := "%0" }))[felts][{ name := "%54" }] ←
            Option.get! (State.felts (st["%52"] ←ₛ getImpl st { name := "data" } 0 14) { name := "%52" }) *
                Option.get! (State.felts st { name := "%0" }) +
              Option.get! (State.felts st { name := "%51" }))[felts][{ name := "%55" }] ←
          Option.get! (State.felts (st["%52"] ←ₛ getImpl st { name := "data" } 0 14) { name := "%52" }) *
                Option.get! (State.felts st { name := "%0" }) +
...
def part13_state_update (st: State): State :=
  Γ (part13_state st) ⟦Code.part14; Code.part15; Code.part16⟧

-- Prove that substituting part13_state for Code.part13 produces the same result
lemma part13_wp {st : State} :
  Code.getReturn (MLIR.runProgram (Code.part13; Code.part14; Code.part15; Code.part16) st) ↔
  Code.getReturn (part13_state_update st) := by
  unfold MLIR.runProgram; simp only
  generalize eq : (Code.part14; Code.part15; Code.part16) = prog
  unfold Code.part13
  MLIR
