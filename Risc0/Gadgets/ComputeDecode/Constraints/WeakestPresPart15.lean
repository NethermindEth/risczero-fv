...
def part15_state (st: State) : State :=
        ...
        ((((st["%59"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ))[felts][{ name := "%62" }] ←
              Option.get! (State.felts st { name := "%61" }) +
                Option.get!
                  (State.felts (st["%59"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ))
                    { name := "%59" }))["%7"] ←ₛ
            getImpl st { name := "in" } (0 : Back) (0 : ℕ))[felts][{ name := "%63" }] ←
          Option.get!
              (State.felts
                (((st["%59"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ))[felts][{ name := "%62" }] ←
                    Option.get! (State.felts st { name := "%61" }) +
