...
def part8_state (st: State) : State :=
        ...
        ((((st["%32"] ←ₛ getImpl st { name := "data" } (0 : Back) (11 : ℕ))[felts][{ name := "%38" }] ←
              Option.get! (State.felts st { name := "%37" }) +
                Option.get!
                  (State.felts (st["%32"] ←ₛ getImpl st { name := "data" } (0 : Back) (11 : ℕ))
                    { name := "%32" }))[felts][{ name := "%39" }] ←
            (Option.get! (State.felts st { name := "%37" }) +
                Option.get!
                  (State.felts (st["%32"] ←ₛ getImpl st { name := "data" } (0 : Back) (11 : ℕ)) { name := "%32" })) *
