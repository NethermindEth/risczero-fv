...
def part8_state (st: State) : State :=
        ...
        ((((st["%32"] ←ₛ
              ...
              (st.felts { name := "%37" : FeltVar }).get! +
                ((st["%32"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (11 : ℕ)).felts
                    { name := "%32" : FeltVar }).get!)[felts][{ name := "%39" : FeltVar }] ←
            ((st.felts { name := "%37" : FeltVar }).get! +
                ((st["%32"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (11 : ℕ)).felts
                    { name := "%32" : FeltVar }).get!) *
              (st.felts { name := "%1" : FeltVar }).get!)[felts][{ name := "%40" : FeltVar }] ←
          ((st.felts { name := "%37" : FeltVar }).get! +
                ((st["%32"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (11 : ℕ)).felts
                    { name := "%32" : FeltVar }).get!) *
...
lemma part8_cumulative_wp {in0 in1 in2 in3 data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Felt} :
  ...
  Code.getReturn
      (part8_state_update
        (((((({
            ...
            data4 * (4 : Felt))[felts][{ name := "%37" : FeltVar }] ←
          data12 * (8 : Felt) + data2 * (2 : Felt)))  := by
