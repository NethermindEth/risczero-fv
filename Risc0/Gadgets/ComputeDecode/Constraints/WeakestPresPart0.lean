...
def part0_state (st: State) : State :=
        ...
        ((((st[props][{ name := "%6" }] ← True)[felts][{ name := "%4" }] ← 4)["%13"] ←ₛ
            getImpl st { name := "data" } 0 8)[felts][{ name := "%14" }] ←
          Option.get!
              (State.felts
                (((st[props][{ name := "%6" }] ← True)[felts][{ name := "%4" }] ← 4)["%13"] ←ₛ
                  getImpl st { name := "data" } 0 8)
                { name := "%13" }) *
            4) 
      ...
      (part0_state_update
        {
          ...
          bufferWidths := Map.fromList [({ name := "in" }, 4), ({ name := "data" }, 18)], constraints := [], cycle := 0,
          felts := Map.empty, isFailed := false, props := Map.empty, vars := [{ name := "in" }, { name := "data" }] })  := by
