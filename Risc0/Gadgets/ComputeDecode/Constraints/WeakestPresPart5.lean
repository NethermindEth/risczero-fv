...
def part5_state (st: State) : State :=
        ...
        ((((st["%11"] ←ₛ getImpl st { name := "data" } 0 13)[felts][{ name := "%26" }] ←
              Option.get! (State.felts st { name := "%25" }) +
                Option.get! (State.felts (st["%11"] ←ₛ getImpl st { name := "data" } 0 13) { name := "%11" }))["%10"] ←ₛ
            getImpl st { name := "in" } 0 3)[felts][{ name := "%27" }] ←
          Option.get!
              (State.felts
                (((st["%11"] ←ₛ getImpl st { name := "data" } 0 13)[felts][{ name := "%26" }] ←
                    Option.get! (State.felts st { name := "%25" }) +
                      Option.get!
                        (State.felts (st["%11"] ←ₛ getImpl st { name := "data" } 0 13) { name := "%11" }))["%10"] ←ₛ
                  getImpl st { name := "in" } 0 3)
                { name := "%10" }) -
            (Option.get! (State.felts st { name := "%25" }) +
              Option.get! (State.felts (st["%11"] ←ₛ getImpl st { name := "data" } 0 13) { name := "%11" }))) 
  ...
  Code.getReturn
      (part5_state_update
        (((((({
              ...
              16)[felts][{ name := "%3" }] ←
            2)[felts][{ name := "%25" }] ←
          (y10 * 64 + (y1 * 16 + y9 * 8 + y8 * 4 + y0)) * 2))  := by
