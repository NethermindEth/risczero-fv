...
def part9_state (st: State) : State :=
        ...
        ((((st["%29"] ←ₛ getImpl st { name := "data" } 0 3)[felts][{ name := "%41" }] ←
              Option.get! (State.felts st { name := "%40" }) +
                Option.get! (State.felts (st["%29"] ←ₛ getImpl st { name := "data" } 0 3) { name := "%29" }))["%9"] ←ₛ
            getImpl st { name := "in" } 0 2)[felts][{ name := "%42" }] ←
          Option.get!
              (State.felts
                (((st["%29"] ←ₛ getImpl st { name := "data" } 0 3)[felts][{ name := "%41" }] ←
                    Option.get! (State.felts st { name := "%40" }) +
                      Option.get!
                        (State.felts (st["%29"] ←ₛ getImpl st { name := "data" } 0 3) { name := "%29" }))["%9"] ←ₛ
                  getImpl st { name := "in" } 0 2)
                { name := "%9" }) -
            (Option.get! (State.felts st { name := "%40" }) +
              Option.get! (State.felts (st["%29"] ←ₛ getImpl st { name := "data" } 0 3) { name := "%29" }))) 
  ...
  Code.getReturn
      (part9_state_update
        ((((({
                ...
                4)[felts][{ name := "%1" }] ←
              16)[props][{ name := "%28" }] ←
            x3 - ((y10 * 64 + (y1 * 16 + y9 * 8 + y8 * 4 + y0)) * 2 + y13) = 0)[felts][{ name := "%40" }] ←
          (y12 * 8 + y2 * 2 + y11) * 16 + y4 * 4))  := by
