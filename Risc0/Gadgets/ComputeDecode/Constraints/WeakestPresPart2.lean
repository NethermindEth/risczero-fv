...
def part2_state (st: State) : State :=
        ...
        ((((st["%17"] ←ₛ
              ...
              ((st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ)).felts
                    { name := "%17" : FeltVar }).get! *
                (st.felts { name := "%1" : FeltVar }).get!)[felts][{ name := "%19" : FeltVar }] ←
            ((st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ)).felts
                    { name := "%17" : FeltVar }).get! *
                (st.felts { name := "%1" : FeltVar }).get! +
              (st.felts { name := "%16" : FeltVar }).get!)[felts][{ name := "%20" : FeltVar }] ←
          ((st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ)).felts
                    { name := "%17" : FeltVar }).get! *
                (st.felts { name := "%1" : FeltVar }).get! +
