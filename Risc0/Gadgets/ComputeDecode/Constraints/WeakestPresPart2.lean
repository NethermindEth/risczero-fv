...
def part2_state (st: State) : State :=
        ...
        ((((st["%17"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ))[felts][{ name := "%18" }] ←
              Option.get!
                  (State.felts (st["%17"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ)) { name := "%17" }) *
                Option.get! (State.felts st { name := "%1" }))[felts][{ name := "%19" }] ←
            Option.get!
                  (State.felts (st["%17"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ)) { name := "%17" }) *
                Option.get! (State.felts st { name := "%1" }) +
              Option.get! (State.felts st { name := "%16" }))[felts][{ name := "%20" }] ←
          Option.get! (State.felts (st["%17"] ←ₛ getImpl st { name := "data" } (0 : Back) (1 : ℕ)) { name := "%17" }) *
                Option.get! (State.felts st { name := "%1" }) +
  ...
  Code.getReturn
      (part2_state_update
        (((((({
              ...
              (8 : Felt))[felts][{ name := "%16" }] ←
            y9 * (8 : Felt))[felts][{ name := "%1" }] ←
          (16 : Felt)))  := by
