...
def part2_state (st: State) : State :=
        ...
        ((((st["%17"] ←ₛ
                getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))[felts][{ name := "%18" : FeltVar }] ←
              Option.get!
                  (State.felts (st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%17" : FeltVar }) *
                Option.get! (State.felts st { name := "%1" : FeltVar }))[felts][{ name := "%19" : FeltVar }] ←
            Option.get!
                  (State.felts (st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%17" : FeltVar }) *
                Option.get! (State.felts st { name := "%1" : FeltVar }) +
              Option.get! (State.felts st { name := "%16" : FeltVar }))[felts][{ name := "%20" : FeltVar }] ←
          Option.get!
                  (State.felts (st["%17"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%17" : FeltVar }) *
                Option.get! (State.felts st { name := "%1" : FeltVar }) +
...
lemma part2_cumulative_wp {in0 in1 in2 in3 data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Felt} :
  ...
  Code.getReturn
      (part2_state_update
        (((((({
                  ...
                  (4 : Felt))[felts][{ name := "%14" : FeltVar }] ←
                data8 * (4 : Felt))[felts][{ name := "%2" : FeltVar }] ←
              (8 : Felt))[felts][{ name := "%16" : FeltVar }] ←
            data9 * (8 : Felt))[felts][{ name := "%1" : FeltVar }] ←
          (16 : Felt)))  := by
