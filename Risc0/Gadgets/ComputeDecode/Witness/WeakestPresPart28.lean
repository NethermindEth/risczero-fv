...
def part28_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%61" : FeltVar }] ←
                (st.felts { name := "%60" : FeltVar }).get! +
                  (st.felts { name := "%58" : FeltVar }).get!)[felts][{ name := "%62" : FeltVar }] ←
              ((st.felts { name := "%60" : FeltVar }).get! + (st.felts { name := "%58" : FeltVar }).get!) *
                (st.felts { name := "%15" : FeltVar }).get!)["%63"] ←ₛ
            getImpl st { name := "data" : BufferVar } (0 : Back) (14 : ℕ))[felts][{ name := "%64" : FeltVar }] ←
          ((((st[felts][{ name := "%61" : FeltVar }] ←
                        (st.felts { name := "%60" : FeltVar }).get! +
                          (st.felts { name := "%58" : FeltVar }).get!)[felts][{ name := "%62" : FeltVar }] ←
                      ((st.felts { name := "%60" : FeltVar }).get! + (st.felts { name := "%58" : FeltVar }).get!) *
                        (st.felts { name := "%15" : FeltVar }).get!)["%63"] ←ₛ
                    getImpl st { name := "data" : BufferVar } (0 : Back) (14 : ℕ)).felts
                { name := "%63" : FeltVar }).get! *
...
lemma part28_wp {st : State} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 : Option Felt} :
  ...
  Code.getReturn (part28_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
...
lemma part28_updates_opaque {st : State} : 
  Code.getReturn (part27_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) ↔
  Code.getReturn (part28_state_update (part27_drops (part27_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
  try simp [part27_state_update, part28_wp]

lemma part28_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
  ...
  Code.getReturn
      (part28_state_update
        ((({
            ...
            feltBitAnd in1 (64 : Felt) * (1981808641 : Felt) * (4 : Felt))[felts][{ name := "%58" : FeltVar }] ←
