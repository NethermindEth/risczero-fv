            ...
            Option.get!
                (State.felts
                  (((st[felts][{ name := "%61" : FeltVar }] ←
                        Option.get! (State.felts st { name := "%60" : FeltVar }) +
                          Option.get!
                            (State.felts st { name := "%58" : FeltVar }))[felts][{ name := "%62" : FeltVar }] ←
                      (Option.get! (State.felts st { name := "%60" : FeltVar }) +
                          Option.get! (State.felts st { name := "%58" : FeltVar })) *
                        Option.get! (State.felts st { name := "%15" : FeltVar }))["%63"] ←ₛ
  ...
  Code.getReturn
        (part28_state_update
          ((({
                ...
                feltBitAnd x1 (12 : Felt) * (1509949441 : Felt) * (4 : Felt))[felts][{ name := "%60" : FeltVar }] ←
              feltBitAnd x1 (64 : Felt) * (1981808641 : Felt) * (4 : Felt))[felts][{ name := "%58" : FeltVar }] ←
