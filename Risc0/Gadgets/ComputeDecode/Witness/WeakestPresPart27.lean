...
def part27_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%57" : FeltVar }] ←
                (st.felts { name := "%56" : FeltVar }).get! * (st.felts { name := "%7" : FeltVar }).get!)["%59"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (15 : ℕ))[felts][{ name := "%60" : FeltVar }] ←
            (((st[felts][{ name := "%57" : FeltVar }] ←
                        (st.felts { name := "%56" : FeltVar }).get! *
                          (st.felts { name := "%7" : FeltVar }).get!)["%59"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (15 : ℕ)).felts
                  { name := "%59" : FeltVar }).get! *
              (st.felts { name := "%7" : FeltVar }).get!)["%58"] ←ₛ
