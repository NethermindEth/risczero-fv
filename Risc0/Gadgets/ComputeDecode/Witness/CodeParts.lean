...
def drop30 : MLIRProgram :=   dropfelt ⟨"%21"⟩; dropfelt ⟨"%21"⟩; dropfelt ⟨"%68"⟩; dropfelt ⟨"%19"⟩
def part31 : MLIRProgram :=   "%69" ←ₐ .Get ⟨"data"⟩ 0 17; "%72" ←ₐ .Add ⟨"%71"⟩ ⟨"%69"⟩; "%73" ←ₐ .Sub ⟨"%20"⟩ ⟨"%72"⟩; ?₀ ⟨"%73"⟩
def drop_part_part31 (h1: ⟨"%69"⟩ ≠ x) (h2: ⟨"%72"⟩ ≠ x) (h3: ⟨"%71"⟩ ≠ x) (h4: ⟨"%73"⟩ ≠ x) (h5: ⟨"%20"⟩ ≠ x) (h6: ⟨"%72"⟩ ≠ x) (h7: ⟨"%73"⟩ ≠ x):
  State.buffers (Γ st ⟦dropfelt x; part31⟧) =
  State.buffers (Γ st ⟦part31; dropfelt x⟧) := by
    unfold part31
    rewrite [←MLIR.seq_assoc]
    rewrite [drop_past_get (by trivial)]
    rewrite [MLIR.run_seq_def]
    rewrite [drop_past_add (by trivial) (by trivial) (by trivial)]
    rewrite [MLIR.run_seq_def]
    rewrite [drop_past_sub (by trivial) (by trivial) (by trivial)]
    rewrite [MLIR.run_seq_def]
    rewrite [drop_past_eqz_single (by trivial)]
    rewrite [←MLIR.run_seq_def]
    rewrite [←MLIR.run_seq_def]
    rewrite [←MLIR.run_seq_def]
    rfl
