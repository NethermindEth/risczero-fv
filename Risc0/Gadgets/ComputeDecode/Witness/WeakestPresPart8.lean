...
def part8_state (st: State) : State :=
          ...
          (State.set!
            (((State.set! st { name := "data" : BufferVar } (12 : ℕ)
                  (Option.get! st.felts[{ name := "%86" : FeltVar }]!))[felts][{ name := "%87" : FeltVar }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%22" : FeltVar }))
                  (Option.get! (State.felts st { name := "%17" : FeltVar })))[felts][{ name := "%88" : FeltVar }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%22" : FeltVar }))
...
def part8_drops (st: State) : State :=
  State.dropFelts (State.dropFelts (State.dropFelts (State.dropFelts (State.dropFelts (st) ⟨"%17"⟩) ⟨"%16"⟩) ⟨"%86"⟩) ⟨"%87"⟩) ⟨"%88"⟩
...
lemma part8_updates_opaque {st : State} : 
  ...
  Code.getReturn (part8_state_update (part7_drops (part7_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part7_state_update, part8_wp]

lemma part8_cumulative_wp {x0 x1 x2 x3: Felt} :
  ...
  Code.getReturn
        (part8_state_update
          (({
                buffers :=
                  (Map.empty[{ name := "in" : BufferVar }] ←ₘ
                    ...
                    [[some (feltBitAnd x3 (6 : Felt) * (1006632961 : Felt)),
                        some (feltBitAnd x3 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none, none,
                        some (feltBitAnd x3 (8 : Felt) * (1761607681 : Felt)),
                        some (feltBitAnd x3 (16 : Felt) * (1887436801 : Felt)),
                        some (feltBitAnd x3 (128 : Felt) * (1997537281 : Felt)), none, none,
                        some (feltBitAnd x3 (1 : Felt)), none, none, none, none]],
                bufferWidths :=
                ...
                vars :=
                  [{ name := "in" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%22" : FeltVar }] ←
              x2)[felts][{ name := "%86" : FeltVar }] ←
            feltBitAnd x2 (128 : Felt) * (1997537281 : Felt))) =
