...
def part8_state (st: State) : State :=
          ...
          (State.set!
            (((State.set! st { name := "data" } (12 : ℕ)
                  (Option.get! st.felts[({ name := "%86" }: FeltVar)]!))[felts][{ name := "%87" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%22" }))
                  (Option.get! (State.felts st { name := "%17" })))[felts][{ name := "%88" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%22" }))
          ...
          (({
                ...
                felts :=
                  (((((((Map.empty[{ name := "%19" }] ←ₘ (128 : Felt))[{ name := "%23" }] ←ₘ x3)[{ name := "%18" }] ←ₘ
                              (1997537281 : Felt))[{ name := "%17" }] ←ₘ
                            (96 : Felt))[{ name := "%16" }] ←ₘ
                          (1950351361 : Felt))[{ name := "%15" }] ←ₘ
                        (16 : Felt))[{ name := "%14" }] ←ₘ
                      (1887436801 : Felt))[{ name := "%13" }] ←ₘ
                    (8 : Felt),
                isFailed := false, props := Map.empty,
                vars := [{ name := "in" }, { name := "data" }] }[felts][{ name := "%22" }] ←
              x2)[felts][{ name := "%86" }] ←
