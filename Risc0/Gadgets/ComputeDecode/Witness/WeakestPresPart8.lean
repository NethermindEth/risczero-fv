...
def part8_state (st: State) : State :=
        ...
        ((((st.set! { name := "data" : BufferVar } (12 : ℕ)
                  st.felts[{ name := "%86" : FeltVar }]!.get!)[felts][{ name := "%87" : FeltVar }] ←
                feltBitAnd (st.felts { name := "%22" : FeltVar }).get!
                  (st.felts { name := "%17" : FeltVar }).get!)[felts][{ name := "%88" : FeltVar }] ←
              feltBitAnd (st.felts { name := "%22" : FeltVar }).get! (st.felts { name := "%17" : FeltVar }).get! *
                (st.felts { name := "%16" : FeltVar }).get!).set!
          { name := "data" : BufferVar } (2 : ℕ)
          (feltBitAnd (st.felts { name := "%22" : FeltVar }).get! (st.felts { name := "%17" : FeltVar }).get! *
            (st.felts { name := "%16" : FeltVar }).get!)) 
...
lemma part8_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
  ...
  Code.getReturn
      (part8_state_update
        (({
              buffers :=
                (Map.empty[{ name := "in" : BufferVar }] ←ₘ
                  ...
                  [[some (feltBitAnd in3 (6 : Felt) * (1006632961 : Felt)),
                      ...
                      some (feltBitAnd in3 (128 : Felt) * (1997537281 : Felt)), none, none,
                      some (feltBitAnd in3 (1 : Felt)), none, none, none, none]],
              bufferWidths :=
            ...
            in2)[felts][{ name := "%86" : FeltVar }] ←
          feltBitAnd in2 (128 : Felt) * (1997537281 : Felt)))
