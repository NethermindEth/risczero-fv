          ...
          (withEqZero
            (Option.get! (State.felts st { name := "%20" }) -
              (Option.get! (State.felts st { name := "%71" }) +
                Option.get!
                  (State.felts (st["%69"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ)) { name := "%69" })))
            (((st["%69"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ))[felts][{ name := "%72" }] ←
                Option.get! (State.felts st { name := "%71" }) +
                  Option.get!
                    (State.felts (st["%69"] ←ₛ getImpl st { name := "data" } (0 : Back) (17 : ℕ))
              ...
              constraints :=
                [x1 -
                  ...
                  x3 -
                    ...
                    (0 : Felt)],
              cycle := (0 : ℕ), felts := Map.empty[{ name := "%20" }] ←ₘ x0, isFailed := false, props := Map.empty,
