...
def part15_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%100" : FeltVar }] ←
                  feltBitAnd (st.felts { name := "%21" : FeltVar }).get!
                    (st.felts { name := "%6" : FeltVar }).get!)[felts][{ name := "%101" : FeltVar }] ←
                feltBitAnd (st.felts { name := "%21" : FeltVar }).get! (st.felts { name := "%6" : FeltVar }).get! *
                  (st.felts { name := "%5" : FeltVar }).get!).set!
            { name := "data" : BufferVar } (7 : ℕ)
            (feltBitAnd (st.felts { name := "%21" : FeltVar }).get! (st.felts { name := "%6" : FeltVar }).get! *
              (st.felts { name := "%5" : FeltVar }).get!))[felts][{ name := "%102" : FeltVar }] ←
          feltBitAnd (st.felts { name := "%21" : FeltVar }).get! (st.felts { name := "%4" : FeltVar }).get!) 
...
lemma part15_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
          ...
          buffers :=
            (Map.empty[{ name := "in" : BufferVar }] ←ₘ
              ...
              [[some (feltBitAnd in3 (6 : Felt) * (1006632961 : Felt)),
                  ...
                  some (feltBitAnd in1 (48 : Felt) * (1887436801 : Felt)), none, none,
                  ...
                  some (feltBitAnd in1 (64 : Felt) * (1981808641 : Felt)), none, none]],
          bufferWidths :=
