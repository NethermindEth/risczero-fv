...
def part7_state (st: State) : State :=
          ...
          ((((State.set! st { name := "data" } (13 : ℕ) (Option.get! st.felts[({ name := "%84" }: FeltVar)]!))["%22"] ←ₛ
                getImpl (State.set! st { name := "data" } (13 : ℕ) (Option.get! st.felts[({ name := "%84" }: FeltVar)]!))
                  { name := "in" } (0 : Back) (2 : ℕ))[felts][{ name := "%85" }] ←
              feltBitAnd
                (Option.get!
                  (State.felts
                    ((State.set! st { name := "data" } (13 : ℕ) (Option.get! st.felts[({ name := "%84" }: FeltVar)]!))["%22"] ←ₛ
