...
def part7_state (st: State) : State :=
          ...
          ((((State.set! st { name := "data" : BufferVar } (13 : ℕ)
                  (Option.get! st.felts[{ name := "%84" : FeltVar }]!))["%22"] ←ₛ
                getImpl
                  (State.set! st { name := "data" : BufferVar } (13 : ℕ)
                    (Option.get! st.felts[{ name := "%84" : FeltVar }]!))
                  ...
                  (State.felts
                    ((State.set! st { name := "data" : BufferVar } (13 : ℕ)
                      ...
                      getImpl
                        (State.set! st { name := "data" : BufferVar } (13 : ℕ)
                          (Option.get! st.felts[{ name := "%84" : FeltVar }]!))
                        { name := "in" : BufferVar } (0 : Back) (2 : ℕ))
                    { name := "%22" : FeltVar }))
                (Option.get! (State.felts st { name := "%19" : FeltVar })))[felts][{ name := "%86" : FeltVar }] ←
            feltBitAnd
                (Option.get!
                  (State.felts
                    ((State.set! st { name := "data" : BufferVar } (13 : ℕ)
