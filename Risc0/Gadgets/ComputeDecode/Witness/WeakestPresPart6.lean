...
def part6_state (st: State) : State :=
          ...
          (((State.set!
                (st[felts][{ name := "%83" }] ←
                  Option.get! (State.felts st { name := "%82" }) * Option.get! (State.felts st { name := "%9" }))
                { name := "data" } (0 : ℕ)
                (Option.get! (State.felts st { name := "%82" }) *
                  Option.get! (State.felts st { name := "%9" })))[felts][{ name := "%8" }] ←
              (1 : Felt))[felts][{ name := "%84" }] ←
            feltBitAnd (Option.get! (State.felts st { name := "%23" })) (1 : Felt)) 
  ...
  Code.getReturn
        (part6_state_update
          (({
              ...
              feltBitAnd x3 (6 : Felt))[felts][{ name := "%9" }] ←
            (1006632961 : Felt))) =
