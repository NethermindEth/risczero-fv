...
def part30_state (st: State) : State :=
        ...
        (((withEqZero ((st.felts { name := "%21" : FeltVar }).get! - (st.felts { name := "%67" : FeltVar }).get!)
              (st[felts][{ name := "%68" : FeltVar }] ←
                (st.felts { name := "%21" : FeltVar }).get! - (st.felts { name := "%67" : FeltVar }).get!))["%70"] ←ₛ
            getImpl st { name := "data" : BufferVar } (0 : Back) (16 : ℕ))[felts][{ name := "%71" : FeltVar }] ←
          (((withEqZero ((st.felts { name := "%21" : FeltVar }).get! - (st.felts { name := "%67" : FeltVar }).get!)
                      (st[felts][{ name := "%68" : FeltVar }] ←
                        (st.felts { name := "%21" : FeltVar }).get! -
                          (st.felts { name := "%67" : FeltVar }).get!))["%70"] ←ₛ
