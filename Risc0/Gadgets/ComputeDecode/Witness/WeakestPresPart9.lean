...
def part9_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%89" : FeltVar }] ←
                  feltBitAnd (st.felts { name := "%22" : FeltVar }).get!
                    (st.felts { name := "%15" : FeltVar }).get!)[felts][{ name := "%90" : FeltVar }] ←
                feltBitAnd (st.felts { name := "%22" : FeltVar }).get! (st.felts { name := "%15" : FeltVar }).get! *
                  (st.felts { name := "%14" : FeltVar }).get!).set!
            { name := "data" : BufferVar } (11 : ℕ)
            (feltBitAnd (st.felts { name := "%22" : FeltVar }).get! (st.felts { name := "%15" : FeltVar }).get! *
              (st.felts { name := "%14" : FeltVar }).get!))[felts][{ name := "%6" : FeltVar }] ←
          (12 : Felt)) 
...
lemma part9_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
          ...
          buffers :=
            (Map.empty[{ name := "in" : BufferVar }] ←ₘ
              ...
              [[some (feltBitAnd in3 (6 : Felt) * (1006632961 : Felt)),
                  ...
                  some (feltBitAnd in2 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none,
                  some (feltBitAnd in3 (8 : Felt) * (1761607681 : Felt)),
                  some (feltBitAnd in3 (16 : Felt) * (1887436801 : Felt)),
                  some (feltBitAnd in3 (128 : Felt) * (1997537281 : Felt)), none,
