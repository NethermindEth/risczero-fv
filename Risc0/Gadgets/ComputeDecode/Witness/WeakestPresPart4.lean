...
def part4_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%13" }] ← (8 : Felt))[felts][{ name := "%80" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%23" })) (8 : Felt))[felts][{ name := "%12" }] ←
              (1761607681 : Felt))[felts][{ name := "%81" }] ←
            feltBitAnd (Option.get! (State.felts st { name := "%23" })) (8 : Felt) * (1761607681 : Felt)) 
            ...
            buffers :=
              (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                [[none, some (feltBitAnd x3 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none, none,
                    ...
                    some (feltBitAnd x3 (128 : Felt) * (1997537281 : Felt)), none, none, none, none, none, none, none]],
