...
def part26_state (st: State) : State :=
        ...
        ((withEqZero
            ((st.felts { name := "%22" : FeltVar }).get! -
              ((st.felts { name := "%52" : FeltVar }).get! + (st.felts { name := "%41" : FeltVar }).get!))
            ((st[felts][{ name := "%53" : FeltVar }] ←
                (st.felts { name := "%52" : FeltVar }).get! +
                  (st.felts { name := "%41" : FeltVar }).get!)[felts][{ name := "%54" : FeltVar }] ←
              (st.felts { name := "%22" : FeltVar }).get! -
                ((st.felts { name := "%52" : FeltVar }).get! + (st.felts { name := "%41" : FeltVar }).get!)))["%56"] ←ₛ
...
lemma part26_wp {st : State} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 : Option Felt} :
  ...
  Code.getReturn (part26_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
...
lemma part26_updates_opaque {st : State} : 
  Code.getReturn (part25_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) ↔
  Code.getReturn (part26_state_update (part25_drops (part25_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
  try simp [part25_state_update, part26_wp]

lemma part26_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
      ...
      (part26_state_update
        (({
            ...
            (feltBitAnd in2 (128 : Felt) * (1997537281 : Felt) * (8 : Felt) +
                ...
                (16 : Felt) +
              feltBitAnd in2 (12 : Felt) * (1509949441 : Felt) * (4 : Felt))[felts][{ name := "%41" : FeltVar }] ←
