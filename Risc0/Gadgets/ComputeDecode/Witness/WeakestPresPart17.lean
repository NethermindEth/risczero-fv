...
def part17_state (st: State) : State :=
        ...
        (State.set!
          (((State.set! st { name := "data" : BufferVar } (16 : ℕ)
                (Option.get! st.felts[{ name := "%104" : FeltVar }]!))[felts][{ name := "%0" : FeltVar }] ←
              (127 : Felt))[felts][{ name := "%105" : FeltVar }] ←
            feltBitAnd (Option.get! (State.felts st { name := "%20" : FeltVar })) (127 : Felt))
