...
def part17_state (st: State) : State :=
          ...
          (State.set!
            (((State.set! st { name := "data" } 16
                  (Option.get! st.felts[({ name := "%104" }: FeltVar)]!))[felts][{ name := "%0" }] ←
                127)[felts][{ name := "%105" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%20" })) 127)
            { name := "data" } 17 (feltBitAnd (Option.get! (State.felts st { name := "%20" })) 127)) 
