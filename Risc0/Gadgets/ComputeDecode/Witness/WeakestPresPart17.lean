...
def part17_state (st: State) : State :=
        ...
        ((((st.set! { name := "data" : BufferVar } (16 : ℕ)
                  st.felts[{ name := "%104" : FeltVar }]!.get!)[felts][{ name := "%0" : FeltVar }] ←
                (127 : Felt))[felts][{ name := "%105" : FeltVar }] ←
              feltBitAnd (st.felts { name := "%20" : FeltVar }).get! (127 : Felt)).set!
          { name := "data" : BufferVar } (17 : ℕ)
          (feltBitAnd (st.felts { name := "%20" : FeltVar }).get! (127 : Felt))) 
...
lemma part17_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
  ...
  Code.getReturn
      (part17_state_update
        (({
              ...
              vars :=
                [{ name := "in" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%20" : FeltVar }] ←
            in0)[felts][{ name := "%104" : FeltVar }] ←
          feltBitAnd in0 (128 : Felt) * (1997537281 : Felt)))
