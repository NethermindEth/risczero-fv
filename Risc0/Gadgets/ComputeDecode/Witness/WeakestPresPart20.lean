...
def part20_state (st: State) : State :=
          ...
          ((((st[felts][{ name := "%33" }] ←
                  Option.get! (State.felts st { name := "%32" }) +
                    Option.get! (State.felts st { name := "%27" }))["%25"] ←ₛ
                getImpl st { name := "data" } (0 : Back) (0 : ℕ))[felts][{ name := "%34" }] ←
              Option.get! (State.felts st { name := "%32" }) + Option.get! (State.felts st { name := "%27" }) +
                Option.get!
                  (State.felts
                    ((st[felts][{ name := "%33" }] ←
                        Option.get! (State.felts st { name := "%32" }) +
  ...
  Code.getReturn
        (part20_state_update
          ((({
                ...
                (4 : Felt))[felts][{ name := "%27" }] ←
              feltBitAnd x3 (8 : Felt) * (1761607681 : Felt) * (4 : Felt))[felts][{ name := "%32" }] ←
