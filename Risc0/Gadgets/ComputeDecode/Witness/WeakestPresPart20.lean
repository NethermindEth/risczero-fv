...
def part20_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%33" : FeltVar }] ←
                (st.felts { name := "%32" : FeltVar }).get! + (st.felts { name := "%27" : FeltVar }).get!)["%25"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (0 : ℕ))[felts][{ name := "%34" : FeltVar }] ←
            (st.felts { name := "%32" : FeltVar }).get! + (st.felts { name := "%27" : FeltVar }).get! +
              (((st[felts][{ name := "%33" : FeltVar }] ←
                        (st.felts { name := "%32" : FeltVar }).get! +
                          (st.felts { name := "%27" : FeltVar }).get!)["%25"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (0 : ℕ)).felts
                  { name := "%25" : FeltVar }).get!)["%35"] ←ₛ
