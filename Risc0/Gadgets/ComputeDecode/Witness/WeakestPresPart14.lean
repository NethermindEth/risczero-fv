...
def part14_state (st: State) : State :=
          ...
          ((State.set!
                (st[felts][{ name := "%95" }] ←
                  Option.get! (State.felts st { name := "%94" }) * Option.get! (State.felts st { name := "%18" }))
                { name := "data" } 14
                (Option.get! (State.felts st { name := "%94" }) *
                  Option.get! (State.felts st { name := "%18" }))[felts][{ name := "%96" }] ←
              ↑(Bitvec.toNat
                  (Bitvec.and (Bitvec.ofNat 256 (ZMod.val (Option.get! (State.felts st { name := "%21" }))))
                    (Bitvec.ofNat 256
                      (ZMod.val (Option.get! (State.felts st { name := "%3" })))))))[felts][{ name := "%97" }] ←
            ↑(Bitvec.toNat
                  (Bitvec.and (Bitvec.ofNat 256 (ZMod.val (Option.get! (State.felts st { name := "%21" }))))
                    (Bitvec.ofNat 256 (ZMod.val (Option.get! (State.felts st { name := "%3" })))))) *
              Option.get!
                (State.felts st
                  {
                    name :=
...
def part14_state_update (st: State): State :=
  Γ (part14_state st) ⟦Code.part15; Code.part16; Code.part17; Code.part18; Code.part19; Code.part20; Code.part21; Code.part22; Code.part23; Code.part24; Code.part25; Code.part26; Code.part27; Code.part28; Code.part29; Code.part30; Code.part31⟧
...
lemma part14_wp {st : State} {y0 y1 y2 y3 y4 y5 y6 y7 y8 y9 y10 y11 y12 y13 y14 y15 y16 y17 : Option Felt} :
  ...
  generalize eq : (Code.part15; Code.part16; Code.part17; Code.part18; Code.part19; Code.part20; Code.part21; Code.part22; Code.part23; Code.part24; Code.part25; Code.part26; Code.part27; Code.part28; Code.part29; Code.part30; Code.part31) = prog
  unfold Code.part14
  MLIR
  rewrite [←eq]
...
lemma part14_updates_opaque {st : State} : 
  ...
  Code.getReturn (part14_state_update (part13_state st)) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
