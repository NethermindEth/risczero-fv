...
def part14_state (st: State) : State :=
          ...
          (State.set!
            (((st[felts][{ name := "%1" }] ← (48 : Felt))[felts][{ name := "%98" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%21" })) (48 : Felt))[felts][{ name := "%99" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%21" })) (48 : Felt) *
