...
def part14_state (st: State) : State :=
          ...
          (State.set!
            (((st[felts][{ name := "%1" }] ← 48)[felts][{ name := "%98" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%21" })) 48)[felts][{ name := "%99" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%21" })) 48 *
            ...
            buffers :=
              (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                [[some (feltBitAnd x3 6 * 1006632961), some (feltBitAnd x3 96 * 1950351361),
                    ...
                    none, none, none, some (feltBitAnd x3 8 * 1761607681), some (feltBitAnd x3 16 * 1887436801),
                    some (feltBitAnd x3 128 * 1997537281), some (feltBitAnd x2 16 * 1887436801),
                    some (feltBitAnd x2 128 * 1997537281), some (feltBitAnd x3 1),
                    some (feltBitAnd x1 128 * 1997537281), some (feltBitAnd x1 64 * 1981808641), none, none]],
