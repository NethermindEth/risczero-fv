...
def part16_state (st: State) : State :=
        ...
        ((((st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)["%20"] ←ₛ
              getImpl (st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)
                { name := "in" : BufferVar } (0 : Back) (0 : ℕ))[felts][{ name := "%103" : FeltVar }] ←
            feltBitAnd
              (((st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)["%20"] ←ₛ
                      getImpl
                        (st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)
                        { name := "in" : BufferVar } (0 : Back) (0 : ℕ)).felts
                  { name := "%20" : FeltVar }).get!
              (st.felts { name := "%19" : FeltVar }).get!)[felts][{ name := "%104" : FeltVar }] ←
          feltBitAnd
              (((st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)["%20"] ←ₛ
                      getImpl
                        (st.set! { name := "data" : BufferVar } (6 : ℕ) st.felts[{ name := "%102" : FeltVar }]!.get!)
...
lemma part16_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
            ...
            buffers :=
              (Map.empty[{ name := "in" : BufferVar }] ←ₘ
                ...
                [[some (feltBitAnd in3 (6 : Felt) * (1006632961 : Felt)),
                    ...
                    some (feltBitAnd in1 (48 : Felt) * (1887436801 : Felt)), none,
