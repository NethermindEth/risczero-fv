...
def part16_state (st: State) : State :=
          ...
          ((((State.set! st { name := "data" } (6 : ℕ) (Option.get! st.felts[({ name := "%102" }: FeltVar)]!))["%20"] ←ₛ
                getImpl (State.set! st { name := "data" } (6 : ℕ) (Option.get! st.felts[({ name := "%102" }: FeltVar)]!))
                  { name := "in" } (0 : Back) (0 : ℕ))[felts][{ name := "%103" }] ←
              feltBitAnd
                (Option.get!
                  (State.felts
                    ((State.set! st { name := "data" } (6 : ℕ) (Option.get! st.felts[({ name := "%102" }: FeltVar)]!))["%20"] ←ₛ
