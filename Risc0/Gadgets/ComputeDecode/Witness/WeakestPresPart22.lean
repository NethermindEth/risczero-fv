          ...
          (withEqZero
            (Option.get! (State.felts st { name := "%23" }) -
              (Option.get! (State.felts st { name := "%38" }) +
                Option.get!
                  (State.felts (st["%24"] ←ₛ getImpl st { name := "data" } (0 : Back) (13 : ℕ)) { name := "%24" })))
            (((st["%24"] ←ₛ getImpl st { name := "data" } (0 : Back) (13 : ℕ))[felts][{ name := "%39" }] ←
                Option.get! (State.felts st { name := "%38" }) +
                  Option.get!
                    (State.felts (st["%24"] ←ₛ getImpl st { name := "data" } (0 : Back) (13 : ℕ))
