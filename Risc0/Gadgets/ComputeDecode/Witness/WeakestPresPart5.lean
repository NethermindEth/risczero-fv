...
def part5_state (st: State) : State :=
        ...
        ((((st.set! { name := "data" : BufferVar } (8 : ℕ)
                st.felts[{ name := "%81" : FeltVar }]!.get!)[felts][{ name := "%10" : FeltVar }] ←
              (6 : Felt))[felts][{ name := "%82" : FeltVar }] ←
            feltBitAnd (st.felts { name := "%23" : FeltVar }).get! (6 : Felt))[felts][{ name := "%9" : FeltVar }] ←
          (1006632961 : Felt)) 
...
lemma part5_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
  ...
  Code.getReturn
      (part5_state_update
        (({
              buffers :=
                (Map.empty[{ name := "in" : BufferVar }] ←ₘ
                  ...
                  [[none, some (feltBitAnd in3 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none, none,
                      none, some (feltBitAnd in3 (16 : Felt) * (1887436801 : Felt)),
          ...
          feltBitAnd in3 (8 : Felt) * (1761607681 : Felt)))
