...
def part5_state (st: State) : State :=
          ...
          ((((State.set! st { name := "data" } (8 : ℕ)
                  (Option.get! st.felts[({ name := "%81" }: FeltVar)]!))[felts][{ name := "%10" }] ←
                (6 : Felt))[felts][{ name := "%82" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%23" })) (6 : Felt))[felts][{ name := "%9" }] ←
            (1006632961 : Felt)) 
