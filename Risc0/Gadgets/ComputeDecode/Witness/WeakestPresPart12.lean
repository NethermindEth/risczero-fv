          ...
          ((State.set!
              ((st[felts][{ name := "%94" }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%21" }))
                    (Option.get! (State.felts st { name := "%19" })))[felts][{ name := "%95" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%21" }))
              ...
              (feltBitAnd (Option.get! (State.felts st { name := "%21" }))
                  (Option.get! (State.felts st { name := "%19" })) *
                Option.get! (State.felts st { name := "%18" })))[felts][{ name := "%3" }] ←
            (64 : Felt)) 
          ...
          ({
              ...
              felts :=
                (((((((((Map.empty[{ name := "%19" }] ←ₘ (128 : Felt))[{ name := "%23" }] ←ₘ x3)[{ name := "%18" }] ←ₘ
                        ...
                        x2)[{ name := "%6" }] ←ₘ
                      (12 : Felt))[{ name := "%5" }] ←ₘ
                    (1509949441 : Felt))[{ name := "%4" }] ←ₘ
                  (3 : Felt),
              isFailed := false, props := Map.empty,
              vars := [{ name := "in" }, { name := "data" }] }[felts][{ name := "%21" }] ←
            x1)) =
