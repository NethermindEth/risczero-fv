          ...
          ((State.set!
              ((st[felts][{ name := "%94" : FeltVar }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%21" : FeltVar }))
                    (Option.get! (State.felts st { name := "%19" : FeltVar })))[felts][{ name := "%95" : FeltVar }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%21" : FeltVar }))
              ...
              (feltBitAnd (Option.get! (State.felts st { name := "%21" : FeltVar }))
                  (Option.get! (State.felts st { name := "%19" : FeltVar })) *
                Option.get! (State.felts st { name := "%18" : FeltVar })))[felts][{ name := "%3" : FeltVar }] ←
            (64 : Felt)) 
...
lemma part12_updates_opaque {st : State} : 
  ...
  Code.getReturn (part12_state_update (part11_drops (part11_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part11_state_update, part12_wp]

lemma part12_cumulative_wp {x0 x1 x2 x3: Felt} :
