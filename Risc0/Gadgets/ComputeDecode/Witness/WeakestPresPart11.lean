...
def part11_state (st: State) : State :=
          ...
          ((State.set!
              ((st[felts][{ name := "%4" }] ← (3 : Felt))[felts][{ name := "%93" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%22" })) (3 : Felt))
              { name := "data" } (3 : ℕ)
              (feltBitAnd (Option.get! (State.felts st { name := "%22" })) (3 : Felt)))["%21"] ←ₛ
            getImpl
              (State.set!
                ((st[felts][{ name := "%4" }] ← (3 : Felt))[felts][{ name := "%93" }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%22" })) (3 : Felt))
                { name := "data" } (3 : ℕ) (feltBitAnd (Option.get! (State.felts st { name := "%22" })) (3 : Felt)))
              { name := "in" } (0 : Back) (1 : ℕ)) 
...
lemma part11_updates_opaque {st : State} : 
  ...
  Code.getReturn (part11_state_update (part10_drops (part10_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part10_state_update, part11_wp]

lemma part11_cumulative_wp {x0 x1 x2 x3: Felt} :
            ...
            buffers :=
              (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                [[some (feltBitAnd x3 (6 : Felt) * (1006632961 : Felt)),
                    ...
                    some (feltBitAnd x2 (96 : Felt) * (1950351361 : Felt)), none,
