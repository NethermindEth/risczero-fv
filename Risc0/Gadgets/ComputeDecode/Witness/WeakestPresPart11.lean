...
def part11_state (st: State) : State :=
          ...
          ((State.set!
              ((st[felts][{ name := "%4" }] ← 3)[felts][{ name := "%93" }] ←
                feltBitAnd (Option.get! (State.felts st { name := "%22" })) 3)
              { name := "data" } 3 (feltBitAnd (Option.get! (State.felts st { name := "%22" })) 3))["%21"] ←ₛ
            getImpl
              (State.set!
                ((st[felts][{ name := "%4" }] ← 3)[felts][{ name := "%93" }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%22" })) 3)
                { name := "data" } 3 (feltBitAnd (Option.get! (State.felts st { name := "%22" })) 3))
              { name := "in" } 0 1) 
            ...
            buffers :=
              (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                [[some (feltBitAnd x3 6 * 1006632961), some (feltBitAnd x3 96 * 1950351361),
                    some (feltBitAnd x2 96 * 1950351361), none, some (feltBitAnd x2 12 * 1509949441), none, none, none,
