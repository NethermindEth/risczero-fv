...
lemma part3_updates_opaque {st : State} : 
  ...
  Code.getReturn (part3_state_update (part2_drops (part2_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part2_state_update, part3_wp]

lemma part3_cumulative_wp {x0 x1 x2 x3: Felt} :
  ...
  Code.getReturn
        (part3_state_update
          ({
              buffers :=
                (Map.empty[{ name := "in" }] ←ₘ [[some x0, some x1, some x2, some x3]])[{ name := "data" }] ←ₘ
                  [[none, some (feltBitAnd x3 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none, none,
                      none, none, some (feltBitAnd x3 (128 : Felt) * (1997537281 : Felt)), none, none, none, none, none,
                      none, none]],
              ...
              vars := [{ name := "in" }, { name := "data" }] }[felts][{ name := "%15" }] ←
            (16 : Felt))) =
