...
def part3_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%78" : FeltVar }] ←
                  feltBitAnd (st.felts { name := "%23" : FeltVar }).get!
                    (st.felts { name := "%15" : FeltVar }).get!)[felts][{ name := "%14" : FeltVar }] ←
                (1887436801 : Felt))[felts][{ name := "%79" : FeltVar }] ←
              feltBitAnd (st.felts { name := "%23" : FeltVar }).get! (st.felts { name := "%15" : FeltVar }).get! *
                (1887436801 : Felt)).set!
