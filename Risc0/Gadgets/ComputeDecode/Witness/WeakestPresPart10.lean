...
def part10_state (st: State) : State :=
          ...
          (State.set!
            (((st[felts][{ name := "%91" : FeltVar }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%22" : FeltVar }))
                    (Option.get! (State.felts st { name := "%6" : FeltVar })))[felts][{ name := "%5" : FeltVar }] ←
                (1509949441 : Felt))[felts][{ name := "%92" : FeltVar }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%22" : FeltVar }))
...
lemma part10_updates_opaque {st : State} : 
  Code.getReturn (part9_state_update st) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] ↔
  Code.getReturn (part10_state_update (part9_drops (part9_state st))) = [y0, y1, y2, y3, y4, y5, y6, y7, y8, y9, y10, y11, y12, y13, y14, y15, y16, y17] := by
  simp [part9_state_update, part10_wp]

lemma part10_cumulative_wp {x0 x1 x2 x3: Felt} :
  ...
  Code.getReturn
        (part10_state_update
          ({
              buffers :=
                (Map.empty[{ name := "in" : BufferVar }] ←ₘ
                  ...
                  [[some (feltBitAnd x3 (6 : Felt) * (1006632961 : Felt)),
                      ...
                      some (feltBitAnd x2 (96 : Felt) * (1950351361 : Felt)), none, none, none, none, none,
                      ...
                      some (feltBitAnd x2 (128 : Felt) * (1997537281 : Felt)), some (feltBitAnd x3 (1 : Felt)), none,
                      none, none, none]],
              bufferWidths :=
              ...
              vars :=
                [{ name := "in" : BufferVar }, { name := "data" : BufferVar }] }[felts][{ name := "%6" : FeltVar }] ←
            (12 : Felt))) =
