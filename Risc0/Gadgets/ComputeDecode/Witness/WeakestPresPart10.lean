...
def part10_state (st: State) : State :=
          ...
          (State.set!
            (((st[felts][{ name := "%91" }] ←
                  feltBitAnd (Option.get! (State.felts st { name := "%22" }))
                    (Option.get! (State.felts st { name := "%6" })))[felts][{ name := "%5" }] ←
                (1509949441 : Felt))[felts][{ name := "%92" }] ←
              feltBitAnd (Option.get! (State.felts st { name := "%22" }))
          ...
          ({
              ...
              felts :=
                ((((((Map.empty[{ name := "%19" }] ←ₘ (128 : Felt))[{ name := "%23" }] ←ₘ x3)[{ name := "%18" }] ←ₘ
                          (1997537281 : Felt))[{ name := "%15" }] ←ₘ
                        (16 : Felt))[{ name := "%14" }] ←ₘ
                      (1887436801 : Felt))[{ name := "%13" }] ←ₘ
                    (8 : Felt))[{ name := "%22" }] ←ₘ
                  x2,
              isFailed := false, props := Map.empty,
              vars := [{ name := "in" }, { name := "data" }] }[felts][{ name := "%6" }] ←
            (12 : Felt))) =
