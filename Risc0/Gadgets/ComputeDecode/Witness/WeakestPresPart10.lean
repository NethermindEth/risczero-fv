...
def part10_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%91" : FeltVar }] ←
                  feltBitAnd (st.felts { name := "%22" : FeltVar }).get!
                    (st.felts { name := "%6" : FeltVar }).get!)[felts][{ name := "%5" : FeltVar }] ←
                (1509949441 : Felt))[felts][{ name := "%92" : FeltVar }] ←
              feltBitAnd (st.felts { name := "%22" : FeltVar }).get! (st.felts { name := "%6" : FeltVar }).get! *
                (1509949441 : Felt)).set!
...
lemma part10_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
