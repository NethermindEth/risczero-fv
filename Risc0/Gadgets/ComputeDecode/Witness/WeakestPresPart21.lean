...
def part21_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%36" : FeltVar }] ←
                (st.felts { name := "%35" : FeltVar }).get! *
                  (st.felts { name := "%3" : FeltVar }).get!)[felts][{ name := "%37" : FeltVar }] ←
              (st.felts { name := "%35" : FeltVar }).get! * (st.felts { name := "%3" : FeltVar }).get! +
                (st.felts { name := "%34" : FeltVar }).get!)[felts][{ name := "%11" : FeltVar }] ←
...
lemma part21_wp {st : State} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 : Option Felt} :
  ...
  Code.getReturn (part21_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
...
lemma part21_updates_opaque {st : State} : 
  Code.getReturn (part20_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) ↔
  Code.getReturn (part21_state_update (part20_drops (part20_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
  try simp [part20_state_update, part21_wp]

lemma part21_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
      ...
      (part21_state_update
        ((({
            ...
            feltBitAnd in3 (96 : Felt) * (1950351361 : Felt) * (16 : Felt) +
              ...
              feltBitAnd in3 (6 : Felt) * (1006632961 : Felt))[felts][{ name := "%35" : FeltVar }] ←
          feltBitAnd in3 (128 : Felt) * (1997537281 : Felt)))
