...
def part25_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%50" : FeltVar }] ←
                Option.get! (State.felts st { name := "%49" : FeltVar }) +
                  Option.get! (State.felts st { name := "%44" : FeltVar }))[felts][{ name := "%51" : FeltVar }] ←
              (Option.get! (State.felts st { name := "%49" : FeltVar }) +
                  Option.get! (State.felts st { name := "%44" : FeltVar })) *
                Option.get! (State.felts st { name := "%15" : FeltVar }))[felts][{ name := "%52" : FeltVar }] ←
            (Option.get! (State.felts st { name := "%49" : FeltVar }) +
                  Option.get! (State.felts st { name := "%44" : FeltVar })) *
                Option.get! (State.felts st { name := "%15" : FeltVar }) +
              Option.get! (State.felts st { name := "%43" : FeltVar }))["%41"] ←ₛ
