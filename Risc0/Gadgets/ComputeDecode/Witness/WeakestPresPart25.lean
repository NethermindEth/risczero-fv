...
def part25_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%50" : FeltVar }] ←
                (st.felts { name := "%49" : FeltVar }).get! +
                  (st.felts { name := "%44" : FeltVar }).get!)[felts][{ name := "%51" : FeltVar }] ←
              ((st.felts { name := "%49" : FeltVar }).get! + (st.felts { name := "%44" : FeltVar }).get!) *
                (st.felts { name := "%15" : FeltVar }).get!)[felts][{ name := "%52" : FeltVar }] ←
            ((st.felts { name := "%49" : FeltVar }).get! + (st.felts { name := "%44" : FeltVar }).get!) *
                (st.felts { name := "%15" : FeltVar }).get! +
              (st.felts { name := "%43" : FeltVar }).get!)["%41"] ←ₛ
...
lemma part25_wp {st : State} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17 : Option Felt} :
  ...
  Code.getReturn (part25_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
...
lemma part25_updates_opaque {st : State} : 
  Code.getReturn (part24_state_update st) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) ↔
  Code.getReturn (part25_state_update (part24_drops (part24_state st))) ([data0, data1, data2, data3, data4, data5, data6, data7, data8, data9, data10, data11, data12, data13, data14, data15, data16, data17]) := by
  try simp [part24_state_update, part25_wp]

lemma part25_cumulative_wp {in0 in1 in2 in3: Felt} {data0 data1 data2 data3 data4 data5 data6 data7 data8 data9 data10 data11 data12 data13 data14 data15 data16 data17: Option Felt} :
  ...
  Code.getReturn
      (part25_state_update
        ((({
              ...
              feltBitAnd in2 (12 : Felt) * (1509949441 : Felt) * (4 : Felt))[felts][{ name := "%49" : FeltVar }] ←
            feltBitAnd in2 (128 : Felt) * (1997537281 : Felt) * (8 : Felt) +
              feltBitAnd in2 (96 : Felt) * (1950351361 : Felt) * (2 : Felt))[felts][{ name := "%44" : FeltVar }] ←
