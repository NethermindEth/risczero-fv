...
def Instruction.fromList (l : List Felt) (h : l.length = 18) : Instruction :=
  Instruction.mk 
    (l[10])
    (l[1])
    (l[9])
...
lemma Instruction.fromList_def {l : List Felt} (h : l.length = 18) :
  Instruction.fromList l h = Instruction.mk 
    (l[10])
    (l[1])
    (l[9])
    (l[8])
...
def Instruction.fromWords (words : BufferAtTime) (h : words.length = 4) : Instruction :=
  let word0 := Bitvec.ofNat 32 (words[0]).get!.val
  let word1 := Bitvec.ofNat 32 (words[1]).get!.val
  let word2 := Bitvec.ofNat 32 (words[2]).get!.val
  let word3 := Bitvec.ofNat 32 (words[3]).get!.val
  Instruction.mk 
...
def decoder_direct_spec (input : BufferAtTime) (output : BufferAtTime) : Prop :=
  ∀ h : input.length = 4, output = (Instruction.fromWords input h).toList.map some
...
theorem toWords_fromWords {words : BufferAtTime} (h : words.length = 4)
  (h_isBytes : (∀ (i : Fin 4), isByte (Option.get! (words[i]))))
  (h_isSome : (∀ (i : Fin 4), ∃ x, words[i] = some x)) :
  (Instruction.fromWords words h).toWords.map some = words := by
  unfold Instruction.toWords Instruction.fromWords
  try simp
  apply List.ext_getElem?
  intros n
  rcases n with _ | ⟨_ | ⟨_ | ⟨_ | n ⟩⟩⟩; try simp
  · 
    --have h' : 128 = allOnes 7 + 1 := by try simp
    rw [Bitvec.ushr_and_commute]
    have h' : (Bitvec.ushr (Bitvec.ofNat 32 128) 7) = Bitvec.ofNat 32 1 := by
      ...
      rw [h]
    have h'' : allOnes 1 = 1 := by try simp
    rw [←h''] at h'
    rw [h']
    ...
    have h_2 : 2 ^ 7 = 128 := by try simp
    rw [h_2]
    have h_3 : 127 = allOnes 7 := by try simp
    rw [h_3]
    have h_4 : (↑(Bitvec.toNat (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) / 128 % 2) * 128 : Felt) =
                ↑(Bitvec.toNat (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) / 128 % 2 * 128) := by try simp
    rw [h_4]
    have h_5 : (↑(Bitvec.toNat (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) / 128 % 2 * 128) +
                ↑(Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) (Bitvec.ofNat 32 (allOnes 7))))) =
               (↑((Bitvec.toNat (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) / 128 % 2 * 128) +
                 (Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val (Option.get! (words[0])))) (Bitvec.ofNat 32 (allOnes 7))))) : Felt) := by simp
    rw [h_5]
    rw [byte_destruct₁ (by {
      have h := h_isBytes ⟨0, (by try simp)⟩
      unfold isByte at h
      exact Nat.lt_succ_of_le h
    })]
    rcases (h_isSome ⟨0, (by try simp)⟩) with ⟨x, h'''⟩
    simp at h'''
    rw [h''']
    simp
    ...
    rw [h]; decide
  · try simp
    -- rw [List.getElem!_eq_getElem? (by try simp [h])]
    rcases (h_isSome ⟨1, (by try simp)⟩) with ⟨x, h_isSome⟩ 
    simp at h_isSome
    rw [h_isSome]
    try simp
    ...
    ↑(Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 3))) = (↑((Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 128)) 7)) * 128 +
        ((Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 64)) 6)) * 4 +
          ...
          16 +
      (Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 12)) 2)) * 4 +
    (Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 3)))) : Felt) := by try simp
    rw [h']
    rw [byte_destruct₂ (by {
      have h := h_isBytes ⟨1, (by try simp)⟩
      unfold isByte at h
      simp at h
      rw [h_isSome] at h
      exact Nat.lt_succ_of_le h
  ...
  · try simp
    ...
    try simp
    rcases (h_isSome ⟨2, (by try simp)⟩) with ⟨x, h_isSome⟩ 
    simp at h_isSome
    rw [h_isSome]
    try simp
    ...
    rw [h']
    rw [byte_destruct₃ (by {
      have h := h_isBytes ⟨2, (by try simp)⟩
      unfold isByte at h
      simp at h
      rw [h_isSome] at h
      exact Nat.lt_succ_of_le h
  ...
  · try simp
    -- rw [List.get!_eq_get? (by try simp[h])]
    try simp
    rcases (h_isSome ⟨3, by try simp⟩) with ⟨x, h_isSome⟩ 
    simp at h_isSome
    rw [List.getElem?_eq_getElem] 
    have : words[(↑(3 : Fin (4 : ℕ)) : ℕ)] = words[3] := rfl
    rw [this] at h_isSome
    rw [h_isSome]
    try simp
    ...
    ↑(Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 1))) = (↑(((Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 128)) 7)) * 64 +
        ((Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 96)) 5)) * 16 +
          ...
          (Bitvec.toNat (Bitvec.ushr (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 6)) 1)))) *
      2 +
    (Bitvec.toNat (Bitvec.and (Bitvec.ofNat 32 (ZMod.val x)) (Bitvec.ofNat 32 1)))) : Felt) := by try simp
    rw [h']
    rw [byte_destruct₄ (by {
      have h := h_isBytes ⟨3, (by try simp)⟩
      unfold isByte at h
      simp at h
      have : words[(↑(3 : Fin (4 : ℕ)) : ℕ)] = words[3] := rfl
      rw [this] at h
...
lemma leg_96 {x : Felt} :
  ...
  have h_eq : (Bitvec.ofNat (32 : ℕ) (ZMod.val (96 : Felt))) = Bitvec.or (oneBitSetVec 32 26) (oneBitSetVec 32 25) := by
    ...
    rw [h]
    have h' := @Bitvec.two_bits_set' 32 5 6 (by try simp)
    have h'' : (oneBitSet ↑(5 : Fin (32 : ℕ)) + oneBitSet ↑(6 : Fin (32 : ℕ))) = (oneBitSet 5 + oneBitSet 6) := by try simp; rfl
    rw [←h'', ←h']
    try simp
    rfl
  rw [h_eq]
  ...
  · rw [h]
    ...
    rfl
  · rw [h, ←Bitvec.ushr_toNat]
    rw [Bitvec.toNat_twoBitSetVec_eq_oneBitSet (by try simp)]
    have h_96 : 96 = ZMod.val (96 : Felt) := rfl
    rw [h_96]
    rw [h_eq]
    rw [h]
...
lemma leg_48 {x : Felt} :
  ...
  have h_eq : (Bitvec.ofNat (32 : ℕ) (ZMod.val (48 : Felt))) = Bitvec.or (oneBitSetVec 32 26) (oneBitSetVec 32 27) := by
    ...
    rw [h]
    have h' := @Bitvec.two_bits_set' 32 4 5 (by try simp)
    have h'' : (oneBitSet ↑(4 : Fin (32 : ℕ)) + oneBitSet ↑(5 : Fin (32 : ℕ))) = (oneBitSet 4 + oneBitSet 5) := by try simp; rfl
    rw [←h'', ←h']
    try simp
    rfl
  rw [h_eq]
...
lemma leg_6 {x : Felt} :
  ...
  have h_eq : (Bitvec.ofNat (32 : ℕ) (ZMod.val (6 : Felt))) = Bitvec.or (oneBitSetVec 32 30) (oneBitSetVec 32 29) := by
    ...
    rw [h]
    have h' := @Bitvec.two_bits_set' 32 1 2 (by try simp)
    have h'' : (oneBitSet ↑(1 : Fin (32 : ℕ)) + oneBitSet ↑(2 : Fin (32 : ℕ))) = (oneBitSet 1 + oneBitSet 2) := by try simp
    rw [←h'', ←h']
    try simp
  rw [h_eq]
...
lemma leg_12 {x : Felt} :
  ...
  have h_eq : (Bitvec.ofNat (32 : ℕ) (ZMod.val (12 : Felt))) = Bitvec.or (oneBitSetVec 32 29) (oneBitSetVec 32 28) := by
    ...
    rw [h]
    have h' := @Bitvec.two_bits_set' 32 2 3 (by try simp)
    have h'' : (oneBitSet ↑(2 : Fin (32 : ℕ)) + oneBitSet ↑(3 : Fin (32 : ℕ))) = (oneBitSet 2 + oneBitSet 3) := by try simp; rfl
    rw [←h'', ←h']
    try simp
    rfl
  rw [h_eq]
...
theorem decoder_closed_form_equiv_decoder_direct_spec : 
  ...
  have h' :(ZMod.val (1 : Felt)) = 1 := by rfl
  try simp [h']
  have h' :(ZMod.val (3 : Felt)) = 3 := by rfl
  try simp [h']
  have h' :(ZMod.val (127 : Felt)) = 127 := by rfl
  try simp [h']
...
def constraints_spec (input : List Felt) (output : List Felt) : Prop :=
  input.length = 4 ∧ 
  ∀ h : output.length = 18, input = (Instruction.fromList output h).toWords
