...
def part9_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%39" : FeltVar }] ←
                Option.get! (State.felts st { name := "%38" : FeltVar }) +
                  Option.get! (State.felts st { name := "%27" : FeltVar }))[felts][{ name := "%2" : FeltVar }] ←
              (3 : Felt))[felts][{ name := "%40" : FeltVar }] ←
            (3 : Felt) -
              (Option.get! (State.felts st { name := "%38" : FeltVar }) +
                Option.get! (State.felts st { name := "%27" : FeltVar })))[props][{ name := "%41" : PropVar }] ←
          (Option.get! (State.props st { name := "%26" : PropVar }) ∧
            (3 : Felt) -
                (Option.get! (State.felts st { name := "%38" : FeltVar }) +
