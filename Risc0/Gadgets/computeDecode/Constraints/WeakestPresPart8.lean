...
def part8_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%36" : FeltVar }] ←
                Option.get! (State.felts st { name := "%35" : FeltVar }) +
                  Option.get! (State.felts st { name := "%30" : FeltVar }))[felts][{ name := "%37" : FeltVar }] ←
              (Option.get! (State.felts st { name := "%35" : FeltVar }) +
                  Option.get! (State.felts st { name := "%30" : FeltVar })) *
                Option.get! (State.felts st { name := "%5" : FeltVar }))[felts][{ name := "%38" : FeltVar }] ←
            (Option.get! (State.felts st { name := "%35" : FeltVar }) +
                  Option.get! (State.felts st { name := "%30" : FeltVar })) *
                Option.get! (State.felts st { name := "%5" : FeltVar }) +
              Option.get! (State.felts st { name := "%29" : FeltVar }))["%27"] ←ₛ
