...
def part13_state (st: State) : State :=
        ...
        ((((st["%42"] ←ₛ
              ...
              Option.get! (State.felts st { name := "%53" : FeltVar }) +
                Option.get!
                  (State.felts (st["%42"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (6 : ℕ))
                    { name := "%42" : FeltVar }))[felts][{ name := "%55" : FeltVar }] ←
            Option.get! (State.felts st { name := "%1" : FeltVar }) -
              (Option.get! (State.felts st { name := "%53" : FeltVar }) +
                Option.get!
                  (State.felts (st["%42"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (6 : ℕ))
                    { name := "%42" : FeltVar })))[props][{ name := "%56" : PropVar }] ←
          (Option.get!
              (State.props (st["%42"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (6 : ℕ))
                { name := "%41" : PropVar }) ∧
            Option.get! (State.felts st { name := "%1" : FeltVar }) -
                (Option.get! (State.felts st { name := "%53" : FeltVar }) +
                  Option.get!
