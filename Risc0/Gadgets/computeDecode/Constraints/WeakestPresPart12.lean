...
def part12_state (st: State) : State :=
        ...
        ((((st["%50"] ←ₛ
              ...
              ((st["%50"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (14 : ℕ)).felts
                    { name := "%50" : FeltVar }).get! *
                (st.felts { name := "%4" : FeltVar }).get!)[felts][{ name := "%52" : FeltVar }] ←
            ((st["%50"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (14 : ℕ)).felts
                    { name := "%50" : FeltVar }).get! *
                (st.felts { name := "%4" : FeltVar }).get! +
              (st.felts { name := "%49" : FeltVar }).get!)[felts][{ name := "%53" : FeltVar }] ←
          ((st["%50"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (14 : ℕ)).felts
                    { name := "%50" : FeltVar }).get! *
                (st.felts { name := "%4" : FeltVar }).get! +
              (st.felts { name := "%49" : FeltVar }).get! +
            (st.felts { name := "%44" : FeltVar }).get!) 
