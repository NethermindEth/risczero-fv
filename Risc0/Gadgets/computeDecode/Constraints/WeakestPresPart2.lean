...
def part2_state (st: State) : State :=
        ...
        ((((st["%15"] ←ₛ
              ...
              Option.get!
                  (State.felts (st["%15"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%15" : FeltVar }) *
                Option.get! (State.felts st { name := "%5" : FeltVar }))[felts][{ name := "%17" : FeltVar }] ←
            Option.get!
                  (State.felts (st["%15"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%15" : FeltVar }) *
                Option.get! (State.felts st { name := "%5" : FeltVar }) +
              Option.get! (State.felts st { name := "%14" : FeltVar }))[felts][{ name := "%18" : FeltVar }] ←
          Option.get!
                  (State.felts (st["%15"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (1 : ℕ))
                    { name := "%15" : FeltVar }) *
                Option.get! (State.felts st { name := "%5" : FeltVar }) +
