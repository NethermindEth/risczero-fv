...
def part8_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%18" : FeltVar }] ←
                (st.felts { name := "%17" : FeltVar }).get! + (st.felts { name := "%12" : FeltVar }).get!)["%10"] ←ₛ
              getImpl st { name := "data" : BufferVar } (0 : Back) (0 : ℕ))[felts][{ name := "%19" : FeltVar }] ←
            (st.felts { name := "%17" : FeltVar }).get! + (st.felts { name := "%12" : FeltVar }).get! +
              (((st[felts][{ name := "%18" : FeltVar }] ←
                        (st.felts { name := "%17" : FeltVar }).get! +
                          (st.felts { name := "%12" : FeltVar }).get!)["%10"] ←ₛ
                      getImpl st { name := "data" : BufferVar } (0 : Back) (0 : ℕ)).felts
                  { name := "%10" : FeltVar }).get!)[felts][{ name := "%0" : FeltVar }] ←
          (64 : Felt)) 
