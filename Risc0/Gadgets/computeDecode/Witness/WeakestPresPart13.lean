...
def part13_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%35" : FeltVar }] ←
                (st.felts { name := "%34" : FeltVar }).get! +
                  (st.felts { name := "%29" : FeltVar }).get!)[felts][{ name := "%36" : FeltVar }] ←
              ((st.felts { name := "%34" : FeltVar }).get! + (st.felts { name := "%29" : FeltVar }).get!) *
                (st.felts { name := "%2" : FeltVar }).get!)[felts][{ name := "%37" : FeltVar }] ←
            ((st.felts { name := "%34" : FeltVar }).get! + (st.felts { name := "%29" : FeltVar }).get!) *
                (st.felts { name := "%2" : FeltVar }).get! +
              (st.felts { name := "%28" : FeltVar }).get!)["%26"] ←ₛ
