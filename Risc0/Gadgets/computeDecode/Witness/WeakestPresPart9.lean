...
def part9_state (st: State) : State :=
        ...
        ((((st["%20"] ←ₛ
              ...
              Option.get!
                  (State.felts (st["%20"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ))
                    { name := "%20" : FeltVar }) *
                Option.get! (State.felts st { name := "%0" : FeltVar }))[felts][{ name := "%22" : FeltVar }] ←
            Option.get!
                  (State.felts (st["%20"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ))
                    { name := "%20" : FeltVar }) *
                Option.get! (State.felts st { name := "%0" : FeltVar }) +
              Option.get! (State.felts st { name := "%19" : FeltVar }))[felts][{ name := "%23" : FeltVar }] ←
          (Option.get!
                  (State.felts (st["%20"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (10 : ℕ))
                    { name := "%20" : FeltVar }) *
                Option.get! (State.felts st { name := "%0" : FeltVar }) +
  ...
  Code.getReturn
      (part9_state_update
        ((((({
            ...
            (0 : Felt) * (16 : Felt) + (0 : Felt) * (8 : Felt) + (0 : Felt) * (4 : Felt) +
