...
def part16_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%46" : FeltVar }] ←
                (st.felts { name := "%45" : FeltVar }).get! +
                  (st.felts { name := "%43" : FeltVar }).get!)[felts][{ name := "%47" : FeltVar }] ←
              ((st.felts { name := "%45" : FeltVar }).get! + (st.felts { name := "%43" : FeltVar }).get!) *
                (st.felts { name := "%2" : FeltVar }).get!)[felts][{ name := "%4" : FeltVar }] ←
            (128 : Felt))["%48"] ←ₛ
  ...
  Code.getReturn
      (part16_state_update
        ((({
              ...
              (0 : Felt))[felts][{ name := "%45" : FeltVar }] ←
            (0 : Felt))[felts][{ name := "%43" : FeltVar }] ←
          (0 : Felt)))
