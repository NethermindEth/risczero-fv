...
def part17_state (st: State) : State :=
        ...
        ((((st[felts][{ name := "%49" : FeltVar }] ←
                (st.felts { name := "%48" : FeltVar }).get! *
                  (st.felts { name := "%4" : FeltVar }).get!)[felts][{ name := "%50" : FeltVar }] ←
              (st.felts { name := "%48" : FeltVar }).get! * (st.felts { name := "%4" : FeltVar }).get! +
                (st.felts { name := "%47" : FeltVar }).get!)[felts][{ name := "%51" : FeltVar }] ←
            (st.felts { name := "%48" : FeltVar }).get! * (st.felts { name := "%4" : FeltVar }).get! +
                (st.felts { name := "%47" : FeltVar }).get! +
              (st.felts { name := "%42" : FeltVar }).get!)["%40"] ←ₛ
