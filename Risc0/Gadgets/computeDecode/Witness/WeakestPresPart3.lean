...
def part3_state (st: State) : State :=
        ...
        (State.set!
          (State.set!
            (State.set! (st[felts][{ name := "%6" : FeltVar }] ← (3 : Felt)) { name := "data" : BufferVar } (3 : ℕ)
              (3 : Felt))
            { name := "data" : BufferVar } (14 : ℕ) (Option.get! st.felts[{ name := "%3" : FeltVar }]!))
          { name := "data" : BufferVar } (15 : ℕ) (Option.get! st.felts[{ name := "%3" : FeltVar }]!)) 
