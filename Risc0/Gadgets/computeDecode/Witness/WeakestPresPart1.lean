...
def part1_state (st: State) : State :=
        ...
        ((((st.set! { name := "data" : BufferVar } (8 : ℕ)
                    st.felts[{ name := "%3" : FeltVar }]!.get!)[felts][{ name := "%7" : FeltVar }] ←
                  (2 : Felt)).set!
              { name := "data" : BufferVar } (0 : ℕ) (2 : Felt)).set!
          { name := "data" : BufferVar } (13 : ℕ) st.felts[{ name := "%3" : FeltVar }]!.get!) 
      ...
      (part1_state_update
        {
          ...
          cycle := (0 : ℕ), felts := Map.empty[{ name := "%3" : FeltVar }] ←ₘ (0 : Felt), isFailed := False,
