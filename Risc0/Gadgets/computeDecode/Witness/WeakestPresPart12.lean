...
def part12_state (st: State) : State :=
        ...
        ((((st["%32"] ←ₛ
              ...
              Option.get!
                  (State.felts (st["%32"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (12 : ℕ))
                    { name := "%32" : FeltVar }) *
                Option.get! (State.felts st { name := "%1" : FeltVar }))[felts][{ name := "%34" : FeltVar }] ←
            Option.get!
                  (State.felts (st["%32"] ←ₛ getImpl st { name := "data" : BufferVar } (0 : Back) (12 : ℕ))
                    { name := "%32" : FeltVar }) *
                Option.get! (State.felts st { name := "%1" : FeltVar }) +
              Option.get! (State.felts st { name := "%31" : FeltVar }))["%29"] ←ₛ
  ...
  Code.getReturn
      (part12_state_update
        (({
            ...
            (0 : Felt) * (4 : Felt))[felts][{ name := "%31" : FeltVar }] ←
          (0 : Felt) * (2 : Felt)))
