        ...
        ((withEqZero
            (Option.get! (State.felts st { name := "%6" : FeltVar }) -
              (Option.get! (State.felts st { name := "%37" : FeltVar }) +
                Option.get! (State.felts st { name := "%26" : FeltVar })))
            ((st[felts][{ name := "%38" : FeltVar }] ←
                Option.get! (State.felts st { name := "%37" : FeltVar }) +
                  Option.get! (State.felts st { name := "%26" : FeltVar }))[felts][{ name := "%39" : FeltVar }] ←
              Option.get! (State.felts st { name := "%6" : FeltVar }) -
                (Option.get! (State.felts st { name := "%37" : FeltVar }) +
                  Option.get! (State.felts st { name := "%26" : FeltVar }))))["%41"] ←ₛ
  ...
  Code.getReturn
      (part14_state_update
        (({
            ...
            ((0 : Felt) * (8 : Felt) + (0 : Felt) * (2 : Felt) + (0 : Felt)) * (16 : Felt) +
              (0 : Felt) * (4 : Felt))[felts][{ name := "%26" : FeltVar }] ←
          (3 : Felt)))
