...
lemma is0_witness_closed_form {x y₁ y₂ : Felt} : is0_witness [x] = [y₁, y₂] ↔ (if x = 0 then 1 else 0) = y₁ ∧ (if x = 0 then 0 else x⁻¹) = y₂ := by
  unfold is0_witness
  rw [MLIR.run_Sequence_Assign_collapsible]
  simp only [Op.eval_getInput, List.getD_cons_zero, State.update_val]
  rw [MLIR.run_Sequence_Assign_collapsible]
  simp only [Op.eval_isz, Map.update_get, beq_iff_eq, State.update_val]
  rw [MLIR.run_Sequence_Set_collapsible (if x = 0 then 1 else 0) (by simp only [Map.update_get])]
  simp only
  rw [MLIR.run_Sequence_Assign_collapsible]
  ...
  rw [MLIR.run_set]
  simp only [Map.update_get]
  unfold List.set ; unfold List.set ; simp only [List.cons.injEq, and_true]
