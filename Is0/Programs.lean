...
-- private def Map.findWith_ {β : Type} [BEq β] (keys : List String) (m : Map String β) (v : β) : α := Id.run do
--   for key in keys do
--     if m key = v do return 
...
--   )
--   evalTactic <| ← `(
--     tactic| all_goals try contradiction
--   )
--   evalTactic <| ← `(
--     tactic| all_goals try rfl
--   )
